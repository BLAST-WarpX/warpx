...
* ``geometry.dims`` (`string`)
    ...
    * For ``RCYLINDER``, one dimensional cylinder, radius
    * For ``RSPHERE``, one dimensional sphere, radius
...
* ``<diag_name>.<species_name>.variables`` (list of `strings` separated by spaces, optional)
    ...
    Choices are ``x``, ``y``, ``z`` for the particle positions (3D, RZ, RSPERE), ``x`` and ``z`` in 2D, ``z`` in 1D, ``x`` and ``y`` for RCYLINDER,
