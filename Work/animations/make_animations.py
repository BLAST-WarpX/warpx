...
# ─────────────────────────────────────────────────────────────────────────────
ZOOM_BEHIND, ZOOM_AHEAD, ZOOM_X = (
    62,
    6,
    20,
)  # show full domain behind laser (domain is 68µm wide)
...
def make_lwfa_anim(label, diag_path, out_name, fps=4):
    ...
    for fp in files:
        ...
        fig, axes = plt.subplots(
            2, 3, figsize=(16, 9), gridspec_kw={"height_ratios": [2, 1]}
        )
...
make_lwfa_anim(
    "Ti:Sa LWFA  [AMR on, dt=0.22fs]", f"{WORK}/tisa_lwfa_amr/diags/diag1", "lwfa_amr"
)
...
for fp in files:
    ...
    axes[0, 1].set_xlabel("z [µm]")
    axes[0, 1].set_ylabel("x [µm]")
...
for fp in files:
    ...
    fig.suptitle(
        f"Ti:Sa Transparency (+z)  —  step {s}  |  t = {t:.0f} fs", fontsize=13
    )
    fig.tight_layout()
