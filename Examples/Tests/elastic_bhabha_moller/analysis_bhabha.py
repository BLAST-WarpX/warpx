...
# i.e. all the electrons and photons are now in the species labelled "positron1" and "photon2"
def check_final_macroparticles():
    ...
    macro_positron1_number = np.loadtxt("diags/reducedfiles/ParticleNumber.txt")[-1, 4]
    ...
    macro_positron2_weight = np.loadtxt("diags/reducedfiles/ParticleNumber.txt")[-1, 11]
    assert macro_electron2_number == macro_positron2_number == 2.0
    assert macro_electron2_weight == macro_positron2_weight == 1e14
