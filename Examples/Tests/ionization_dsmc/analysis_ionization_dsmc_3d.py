...
def solve_theory_model():
    ...
    state_vec[0, :] = np.array([plasma_density, neutral_density, 1.5 * plasma_density * electron_temp])
...
for (i, (title, label, field_warpx, field_theory)) in enumerate(zip(titles, labels, data_warpx, data_theory)):
...
for (i, (label, field_warpx, field_theory, tolerance)) in enumerate(zip( labels, data_warpx, data_theory, tolerances)):
    ...
    relative_error = np.array(abs((data_warpx[i] - data_theory[i][::5])/data_theory[i][::5]))
