...
parser.add_argument(
    "--restart", type=str, default=None, help="Restart from checkpoint file"
)
...
def verify_restored_fields():
    ...
    if sim.amr_restart and step == 6:
        ...
        assert abs(scalar_mean - 42.0) < 1e-10, (
            f"Scalar field not restored correctly! Expected 42.0, got {scalar_mean}"
        )
        ...
        for idir, dirstr in enumerate(["x", "y", "z"]):
            ...
            assert abs(vec_mean - expected) < 1e-10, (
                f"Vector {dirstr} not restored correctly! Expected {expected}, got {vec_mean}"
            )
