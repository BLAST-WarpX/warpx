...
x_ele, y_ele, z_ele, uz_ele, w_ele = series.get_particle([ "x", "y", "z", "uz", "w"], species="beam1", iteration=1)
x_pos, y_pos, z_pos, uz_pos, w_pos = series.get_particle([ "x", "y", "z", "uz", "w"], species="beam2", iteration=1)
...
plt.plot(rho_max*np.cos(phi_circ)+x_pos, rho_max*np.sin(phi_circ)+y_pos, "r-", label=r"$\rho_{max}=\frac{\hbar}{m_e c x_{min}}$ [m]")
