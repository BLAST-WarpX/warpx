...
T1 = 1.e6         # 1 MeV, electron energy
m_e_eV = constants.m_e*constants.c**2/constants.q_e # electron rest mass in eV
gamma = T1/m_e_eV + 1. # electron gamma factor
gamma_beta = math.sqrt(gamma**2 - 1.)
...
elec_dist = picmi.UniformDistribution(
    density=n_e,
    directed_velocity=[0., 0., gamma_beta*constants.c],
)
...
sim.add_species(
    ...
    layout=picmi.GriddedLayout(
        n_macroparticle_per_cell=[Np_e], grid=grid
    ),
)
sim.add_species(
    ions,
    layout=picmi.GriddedLayout(
        n_macroparticle_per_cell=[Np_i], grid=grid
    ),
)
sim.add_species(
    photons,
