...
def analyze_boundary_effects(ds, boundary_type):
    ...
    # 提取边界附近的电场数据
    # 选择靠近边界的区域进行分析
    boundary_region = ad.cut_region("obj['index', 'x'] < -0.08 or obj['index', 'x'] > 0.08 or "
...
def plot_field_comparison(quartz_ds, pec_ds):
    ...
    # 石英边界条件
    ax = axes[0, 3]
    plot = yt.SlicePlot(quartz_ds, 'z', ('boxlib', 'rho'), center=[0, 0, 0], width=[0.2, 0.2])
    ...
    # PEC边界条件
    ax = axes[1, 3]
    plot = yt.SlicePlot(pec_ds, 'z', ('boxlib', 'rho'), center=[0, 0, 0], width=[0.2, 0.2])
    plot.set_cmap(('boxlib', 'rho'), 'viridis')
    plot_frb = plot.frb[('boxlib', 'rho')]
    im = ax.imshow(plot_frb, origin='lower')
...
def generate_comparison_table(quartz_ds, pec_ds, output_dir):
    ...
    for field in fields:
        ...
        try:
            ...
            print(f"石英边界 - 范围: {float(quartz_data.min()):.2e} 到 {float(quartz_data.max()):.2e}")
            print(f"PEC边界  - 范围: {float(pec_data.min()):.2e} 到 {float(pec_data.max()):.2e}")
            print(f"差异     - 最大值差异: {float(pec_data.max() - quartz_data.max()):.2e}")
