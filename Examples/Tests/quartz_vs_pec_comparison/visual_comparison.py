...
def create_difference_histograms(quartz_ds, pec_ds, output_dir="comparison_plots"):
    ...
    fig.suptitle('Quartz vs PEC Boundary - Field Difference Histograms', fontsize=16, fontweight='bold')
    ...
    for i, field in enumerate(fields):
        ...
        axes[row, col].axvline(x=0, color='blue', linestyle='--', linewidth=2, label='Zero')
        axes[row, col].set_xlabel(f'{field} Difference')
        axes[row, col].set_ylabel('Frequency')
        axes[row, col].set_title(f'{field}\nMax: {max_diff:.2e}\nMean: {mean_diff:.2e}\nRel: {max_rel_diff:.2f}%')
        axes[row, col].legend()
        axes[row, col].grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(f'{output_dir}/field_differences_histogram.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def create_profile_comparison(quartz_ds, pec_ds, output_dir="comparison_plots"):
    ...
    fig.suptitle('Quartz vs PEC Boundary - Center Line Profiles', fontsize=16, fontweight='bold')
    ...
    for i, field in enumerate(fields):
        ...
        axes[i].plot(x_coords, quartz_field, 'r-', linewidth=2, label='Quartz', alpha=0.8)
        axes[i].plot(x_coords, pec_field, 'b--', linewidth=2, label='PEC', alpha=0.8)
        axes[i].plot(x_coords, quartz_field - pec_field, 'g-', linewidth=1, label='Diff', alpha=0.6)
...
def create_statistics_summary(quartz_ds, pec_ds, output_dir="comparison_plots"):
    ...
    fig.suptitle('Quartz vs PEC Boundary - Statistics Summary', fontsize=16, fontweight='bold')
    ...
    with open(f'{output_dir}/statistics_table.txt', 'w') as f:
        ...
        f.write("=" * 40 + "\n\n")
        f.write(f"{'Field':<8} {'Max Diff':<12} {'Mean Diff':<12} {'Max Rel(%)':<12} {'Corr':<8}\n")
        f.write("-" * 60 + "\n")
        for stats in stats_data:
            f.write(f"{stats['field']:<8} {stats['max_diff']:<12.2e} {stats['mean_diff']:<12.2e} "
