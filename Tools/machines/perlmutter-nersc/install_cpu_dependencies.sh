...
rm -rf $HOME/src/c-blosc2-pm-cpu-build
cmake -S $HOME/src/c-blosc2 -B ${build_dir}/c-blosc2-pm-cpu-build -DBUILD_STATIC=OFF -DBUILD_TESTS=OFF -DBUILD_FUZZERS=OFF -DBUILD_BENCHMARKS=OFF -DBUILD_EXAMPLES=OFF -DDEACTIVATE_AVX2=OFF -DCMAKE_INSTALL_PREFIX=${SW_DIR}/c-blosc2-2.23.1
cmake --build ${build_dir}/c-blosc2-pm-cpu-build --target install --parallel ${PARALLEL}
rm -rf ${build_dir}/c-blosc2-pm-cpu-build
...
cmake -S $HOME/src/adios2 -B ${build_dir}/adios2-pm-cpu-build -DADIOS2_USE_Blosc2=ON -DADIOS2_USE_CUDA=OFF -DADIOS2_USE_Fortran=OFF -DADIOS2_USE_Python=OFF -DADIOS2_USE_ZeroMQ=OFF -DCMAKE_INSTALL_PREFIX=${SW_DIR}/adios2-2.12.1
