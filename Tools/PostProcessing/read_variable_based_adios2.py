...
def get_particle_data(diag_path, species_name, variables, n_iterations=-1, n_particles=-1):
    """
    ...
    *argv : tuple (numpy array (n_iterations, n_particles) , 2-numbers array-like)
    ...
    for arg in argv:
        ...
        if arg[0].shape != ids.shape:
            raise ValueError(f'Dimensions not matching')
        try:
            if len(arg[1]) != 2:
                raise ValueError(f'limits should contain two values, but has {len(arg[1])}')
        except TypeError:
            raise TypeError(f'Expected a 2 elements container, got {type(arg[1])}')

        x_it = (arg[0])[iteration, :]
        selected_ids = id_it[ np.isin(id_it, selected_ids) &
                            (x_it >= (arg[1])[0]) &
                            (x_it <= (arg[1])[1]) 
        ]
