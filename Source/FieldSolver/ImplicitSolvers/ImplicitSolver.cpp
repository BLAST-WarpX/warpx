...
void ImplicitSolver::DiagCompareMMJacobian ( const amrex::Real a_cur_time )
{
    using namespace amrex::literals;
    using warpx::fields::FieldType;
    ...
    s_ncalls++;
    // Sample only the first matvec of each linear solve: its Krylov vector is the
    // (normalized) Newton residual, i.e. the direction the Newton error lives in.
    if (!m_diag_first_matvec) { return; }
    m_diag_first_matvec = false;
    static int s_nsolves = 0;
    s_nsolves++;
    if ((s_nsolves - 1) % m_diag_mm_interval != 0) { return; }

    const int lev = 0;

    ablastr::fields::VectorField J = m_WarpX->m_fields.get_alldirs(FieldType::current_fp, lev);
    ablastr::fields::VectorField J0 = m_WarpX->m_fields.get_alldirs(FieldType::current_fp_non_suborbit, lev);
    amrex::MultiFab* rho = (m_WarpX->m_fields.has(FieldType::rho_fp, lev) ?
                            m_WarpX->m_fields.get(FieldType::rho_fp, lev) : nullptr);

    // Save the raw mass-matrix J and every container the reference push overwrites
    std::array<std::unique_ptr<amrex::MultiFab>, 3> Jmm, J0_save;
    for (int n = 0; n < 3; ++n) {
        Jmm[n] = std::make_unique<amrex::MultiFab>(J[n]->boxArray(), J[n]->DistributionMap(),
                                                   J[n]->nComp(), J[n]->nGrowVect());
        amrex::MultiFab::Copy(*Jmm[n], *J[n], 0, 0, J[n]->nComp(), J[n]->nGrowVect());
        J0_save[n] = std::make_unique<amrex::MultiFab>(J0[n]->boxArray(), J0[n]->DistributionMap(),
                                                       J0[n]->nComp(), J0[n]->nGrowVect());
        amrex::MultiFab::Copy(*J0_save[n], *J0[n], 0, 0, J0[n]->nComp(), J0[n]->nGrowVect());
    }
    std::unique_ptr<amrex::MultiFab> rho_save;
    if (rho) {
        rho_save = std::make_unique<amrex::MultiFab>(rho->boxArray(), rho->DistributionMap(),
                                                     rho->nComp(), rho->nGrowVect());
        amrex::MultiFab::Copy(*rho_save, *rho, 0, 0, rho->nComp(), rho->nGrowVect());
    }

    // Reference: the conventional particle-suppressed JFNK evaluation of J for the same E
    ImplicitOptions options;
    options.linear_stage_of_jfnk = true;
    options.evolve_suborbit_particles_only = false;
    options.use_mass_matrices_jacobian = false;
    options.use_mass_matrices_pc = false;
    options.use_rho_non_suborbit = false;
    options.max_particle_iterations = m_max_particle_iterations;
    options.particle_tolerance = m_particle_tolerance;
    m_WarpX->PushParticlesandDeposit(a_cur_time, false, PositionPushType::Full, MomentumPushType::Full, &options);
    CumulateJ();

    // Save the conventional J at the perturbed E
    std::array<std::unique_ptr<amrex::MultiFab>, 3> Jref;
    for (int n = 0; n < 3; ++n) {
        Jref[n] = std::make_unique<amrex::MultiFab>(J[n]->boxArray(), J[n]->DistributionMap(),
                                                    J[n]->nComp(), J[n]->nGrowVect());
        amrex::MultiFab::Copy(*Jref[n], *J[n], 0, 0, J[n]->nComp(), J[n]->nGrowVect());
    }

    // Save the reference rho at the perturbed E before the second push overwrites it
    std::unique_ptr<amrex::MultiFab> rho_pert;
    if (rho) {
        rho_pert = std::make_unique<amrex::MultiFab>(rho->boxArray(), rho->DistributionMap(),
                                                     rho->nComp(), rho->nGrowVect());
        amrex::MultiFab::Copy(*rho_pert, *rho, 0, 0, rho->nComp(), rho->nGrowVect());
    }

    // Second reference push at the unperturbed E0 (Efield_fp_save), to isolate the
    // Jacobian response dJ = J(E0 + eps*v) - J(E0) from the O(1) base current.
    ablastr::fields::VectorField E = m_WarpX->m_fields.get_alldirs(FieldType::Efield_fp, lev);
    const ablastr::fields::VectorField E0 = m_WarpX->m_fields.get_alldirs(FieldType::Efield_fp_save, lev);
    std::array<std::unique_ptr<amrex::MultiFab>, 3> E_save;
    for (int n = 0; n < 3; ++n) {
        E_save[n] = std::make_unique<amrex::MultiFab>(E[n]->boxArray(), E[n]->DistributionMap(),
                                                      E[n]->nComp(), E[n]->nGrowVect());
        amrex::MultiFab::Copy(*E_save[n], *E[n], 0, 0, E[n]->nComp(), E[n]->nGrowVect());
        amrex::MultiFab::Copy(*E[n], *E0[n], 0, 0, E[n]->nComp(), E[n]->nGrowVect());
    }
    m_WarpX->ApplyFillBoundaryE();
    m_WarpX->PushParticlesandDeposit(a_cur_time, false, PositionPushType::Full, MomentumPushType::Full, &options);
    CumulateJ();
    // Now J holds J_conv(E0). Restore the perturbed E.
    for (int n = 0; n < 3; ++n) {
        amrex::MultiFab::Copy(*E[n], *E_save[n], 0, 0, E[n]->nComp(), E[n]->nGrowVect());
    }
    m_WarpX->ApplyFillBoundaryE();

    // Form the responses in place:
    //   Jref <- J_conv(E) - J_conv(E0)     (true Jacobian response)
    //   Jmm  <- J_mm - J0                   (mass-matrix response; J_suborbit assumed
    //                                        negligible/identical between the two)
    for (int n = 0; n < 3; ++n) {
        amrex::MultiFab::Subtract(*Jref[n], *J[n], 0, 0, J[n]->nComp(), J[n]->nGrowVect());
        amrex::MultiFab::Subtract(*Jmm[n], *J0_save[n], 0, 0, J[n]->nComp(), J[n]->nGrowVect());
    }

    // Report the response mismatch (raw, pre volume scaling), including guard rows,
    // versus the first (radial in RZ) index.
    const char* names[3] = {"Jx/Jr", "Jy/Jt", "Jz   "};
    amrex::Print() << "MMdiag: step " << m_WarpX->getistep(0)
                   << ", linear-stage call " << s_ncalls << "\n";
    for (int n = 0; n < 3; ++n) {
        const amrex::MultiFab& ref = *Jref[n];
        const amrex::MultiFab& mm  = *Jmm[n];
        const amrex::IntVect ng = ref.nGrowVect();

        // host copies
        amrex::MultiFab ref_h(ref.boxArray(), ref.DistributionMap(), ref.nComp(), ng,
                              amrex::MFInfo().SetArena(amrex::The_Pinned_Arena()));
        amrex::MultiFab mm_h(mm.boxArray(), mm.DistributionMap(), mm.nComp(), ng,
                             amrex::MFInfo().SetArena(amrex::The_Pinned_Arena()));
        amrex::dtoh_memcpy(ref_h, ref);
        amrex::dtoh_memcpy(mm_h, mm);

        const amrex::Box dom = amrex::convert(m_WarpX->Geom(lev).Domain(), ref.ixType().toIntVect());
        const int ilo = dom.smallEnd(0) - ng[0];
        const int ihi = dom.bigEnd(0) + ng[0];
        const int nbins = ihi - ilo + 1;
        std::vector<amrex::Real> d2(nbins, 0.0), r2(nbins, 0.0);

        for (amrex::MFIter mfi(ref_h, false); mfi.isValid(); ++mfi) {
            amrex::Box bx = mfi.validbox();
            bx.grow(ng);
            bx &= amrex::grow(dom, ng);
            const auto ra = ref_h.const_array(mfi);
            const auto ma = mm_h.const_array(mfi);
            amrex::LoopOnCpu(bx, [&] (int i, int j, int k) {
                const amrex::Real d = ra(i,j,k,0) - ma(i,j,k,0);
                d2[i - ilo] += d*d;
                r2[i - ilo] += ra(i,j,k,0)*ra(i,j,k,0);
            });
        }
        amrex::ParallelDescriptor::ReduceRealSum(d2.data(), nbins);
        amrex::ParallelDescriptor::ReduceRealSum(r2.data(), nbins);

        amrex::Real d2tot = 0.0, r2tot = 0.0;
        for (int b = 0; b < nbins; ++b) { d2tot += d2[b]; r2tot += r2[b]; }
        amrex::Print() << "  " << names[n]
                       << "  response rel_l2 = "
                       << (r2tot > 0. ? std::sqrt(d2tot/r2tot) : 0.)
                       << "  (||dJ_ref||_l2 = " << std::sqrt(r2tot)
                       << ", ||J(E0)||_l2 = " << J[n]->norm2(0) << ")\n";
        amrex::Print() << "    i (from " << ilo << "): rel per radial row:";
        for (int b = 0; b < nbins; ++b) {
            if (b % 8 == 0) { amrex::Print() << "\n      "; }
            const amrex::Real rel = (r2[b] > 0. ? std::sqrt(d2[b]/r2[b]) : 0.);
            amrex::Print() << ilo + b << ":" << std::setprecision(2) << rel << " ";
        }
        amrex::Print() << std::setprecision(8) << "\n";
    }

    // Also compare the responses after the RZ inverse-volume scaling and guard folding
    // (both linear operations), restricted to valid rows: this measures what the
    // guard-row mismatches turn into after they are folded into the wall/axis rows.
#if defined(WARPX_DIM_RZ) || defined(WARPX_DIM_RCYLINDER) || defined(WARPX_DIM_RSPHERE)
    {
        auto fold = [&](std::array<std::unique_ptr<amrex::MultiFab>, 3>& V) {
            for (int n = 0; n < 3; ++n) {
                amrex::MultiFab::Copy(*J[n], *V[n], 0, 0, J[n]->nComp(), J[n]->nGrowVect());
            }
            m_WarpX->ApplyInverseVolumeScalingToCurrentDensity(J[0], J[1], J[2], lev);
            m_WarpX->SyncCurrentAndRho();
            for (int n = 0; n < 3; ++n) {
                amrex::MultiFab::Copy(*V[n], *J[n], 0, 0, J[n]->nComp(), J[n]->nGrowVect());
            }
        };
        fold(Jref);
        fold(Jmm);
        const char* fnames[3] = {"Jr fold", "Jt fold", "Jz fold"};
        for (int n = 0; n < 3; ++n) {
            amrex::MultiFab d(Jref[n]->boxArray(), Jref[n]->DistributionMap(), 1, 0);
            amrex::MultiFab::Copy(d, *Jref[n], 0, 0, 1, 0);
            amrex::MultiFab::Subtract(d, *Jmm[n], 0, 0, 1, 0);
            const amrex::Real dl2 = d.norm2(0);
            const amrex::Real rl2 = Jref[n]->norm2(0);
            amrex::Print() << "  " << fnames[n] << "ed response rel_l2 (valid) = "
                           << (rl2 > 0. ? dl2/rl2 : 0.) << "\n";
        }
    }
#endif

    // Neglected rho response: the reference push deposited rho at particle positions
    // that respond to the perturbed E, while the mass-matrix linear stage freezes rho.
    // Compare against both the frozen rho and the perturbation scale of E.
    if (rho) {
        const int nc = rho->nComp()/2;
        const int c_new = rho->nComp() - nc;
        // rho currently holds the reference deposit at E0; rho_save holds the frozen
        // linear-stage rho; rho_pert holds the reference deposit at E0 + eps*v.
        // base = state inconsistency at the same field; resp = true E-response.
        amrex::MultiFab dbase(rho->boxArray(), rho->DistributionMap(), 1, 0);
        amrex::MultiFab::Copy(dbase, *rho, c_new, 0, 1, 0);
        amrex::MultiFab::Subtract(dbase, *rho_save, c_new, 0, 1, 0);
        amrex::MultiFab dresp(rho->boxArray(), rho->DistributionMap(), 1, 0);
        amrex::MultiFab::Copy(dresp, *rho_pert, c_new, 0, 1, 0);
        amrex::MultiFab::Subtract(dresp, *rho, c_new, 0, 1, 0);
        const amrex::Real rho_ref_l2 = rho->norm2(c_new);
        amrex::Print() << "  rho base mismatch (same E): ||rho_frozen - rho_ref(E0)||/||rho_ref(E0)|| = "
                       << (rho_ref_l2 > 0. ? dbase.norm2(0)/rho_ref_l2 : 0.) << "\n";
        amrex::Print() << "  rho true response:          ||rho_ref(E+ev) - rho_ref(E0)||/||rho_ref(E0)|| = "
                       << (rho_ref_l2 > 0. ? dresp.norm2(0)/rho_ref_l2 : 0.) << "\n";
        // radial profile of the base mismatch
        {
            amrex::MultiFab dbase_h(dbase.boxArray(), dbase.DistributionMap(), 1, 0,
                                    amrex::MFInfo().SetArena(amrex::The_Pinned_Arena()));
            amrex::MultiFab ref_h(rho->boxArray(), rho->DistributionMap(), rho->nComp(), 0,
                                  amrex::MFInfo().SetArena(amrex::The_Pinned_Arena()));
            amrex::dtoh_memcpy(dbase_h, dbase);
            amrex::dtoh_memcpy(ref_h, *rho);
            const amrex::Box dom = amrex::convert(m_WarpX->Geom(lev).Domain(),
                                                  rho->ixType().toIntVect());
            const int ilo = dom.smallEnd(0);
            const int nbins = dom.length(0);
            std::vector<amrex::Real> d2(nbins, 0.0), r2(nbins, 0.0);
            for (amrex::MFIter mfi(dbase_h, false); mfi.isValid(); ++mfi) {
                const amrex::Box bx = mfi.validbox();
                const auto da = dbase_h.const_array(mfi);
                const auto ra = ref_h.const_array(mfi);
                amrex::LoopOnCpu(bx, [&] (int i, int j, int k) {
                    d2[i - ilo] += da(i,j,k,0)*da(i,j,k,0);
                    r2[i - ilo] += ra(i,j,k,c_new)*ra(i,j,k,c_new);
                });
            }
            amrex::ParallelDescriptor::ReduceRealSum(d2.data(), nbins);
            amrex::ParallelDescriptor::ReduceRealSum(r2.data(), nbins);
            amrex::Print() << "    rho base mismatch, radial rel profile:";
            for (int b = 0; b < nbins; ++b) {
                if (b % 8 == 0) { amrex::Print() << "\n      "; }
                const amrex::Real rel = (r2[b] > 0. ? std::sqrt(d2[b]/r2[b]) : 0.);
                amrex::Print() << ilo + b << ":" << std::setprecision(2) << rel << " ";
        ...
        amrex::MultiFab drho(rho->boxArray(), rho->DistributionMap(), 1, 0);
        amrex::MultiFab::Copy(drho, *rho_pert, c_new, 0, 1, 0);
        amrex::MultiFab::Subtract(drho, *rho_save, c_new, 0, 1, 0);
        const amrex::Real drho_l2 = drho.norm2(0);
        const amrex::Real rho_l2  = rho_save->norm2(c_new);
        // relative perturbation of E driving this response
        amrex::Real dE2 = 0._rt, E2 = 0._rt;
        for (int n = 0; n < 3; ++n) {
            amrex::MultiFab dE(E[n]->boxArray(), E[n]->DistributionMap(), 1, 0);
            amrex::MultiFab::Copy(dE, *E[n], 0, 0, 1, 0);
            amrex::MultiFab::Subtract(dE, *E0[n], 0, 0, 1, 0);
            dE2 += std::pow(dE.norm2(0), 2);
            E2  += std::pow(E0[n]->norm2(0), 2);
        }
        const amrex::Real rel_drho = (rho_l2 > 0. ? drho_l2/rho_l2 : 0.);
        const amrex::Real rel_dE   = (E2 > 0. ? std::sqrt(dE2/E2) : 0.);
        amrex::Print() << "  rho   response: ||drho||/||rho|| = " << rel_drho
                       << "  ||dE||/||E0|| = " << rel_dE
                       << "  gain (drho/rho)/(dE/E) = "
                       << (rel_dE > 0. ? rel_drho/rel_dE : 0.) << "\n";
    }

    // Restore all containers so the solver continues on the mass-matrix path
    for (int n = 0; n < 3; ++n) {
        amrex::MultiFab::Copy(*J[n], *Jmm[n], 0, 0, J[n]->nComp(), J[n]->nGrowVect());
        amrex::MultiFab::Copy(*J0[n], *J0_save[n], 0, 0, J0[n]->nComp(), J0[n]->nGrowVect());
    }
    if (rho) {
        amrex::MultiFab::Copy(*rho, *rho_save, 0, 0, rho->nComp(), rho->nGrowVect());
    }
}
...
void ImplicitSolver::parseNonlinearSolverParams ( const amrex::ParmParse&  pp )
{
    ...
    if (m_nlsolver_type == NonlinearSolverType::picard) {
        ...
        pp.query("diag_mm_jacobian", m_diag_mm_jacobian);
        pp.query("diag_mm_interval", m_diag_mm_interval);
                                ...
                                const bool         a_from_jacobian )
{
    ...
    if (m_diag_mm_jacobian && m_use_mass_matrices_jacobian && a_from_jacobian) {
        // Compare the raw (pre volume scaling) mass-matrix J against the conventional
        // particle-computed J for the same E state.
        DiagCompareMMJacobian( a_cur_time );
    }
