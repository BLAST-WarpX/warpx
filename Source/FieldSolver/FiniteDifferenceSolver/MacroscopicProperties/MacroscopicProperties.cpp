...
{
    //MacroscopicProperties can be used with Maxwell or lab frame electrostatic
    if (WarpX::electrostatic_solver_id != ElectrostaticSolverAlgo::None) {
        m_do_electrostatic = true;
    }
    const ParmParse pp_macroscopic("macroscopic");
    // If macroscopic maxwell solve is turned on,
    ...
    const amrex::IntVect& ng_EB_alloc )
{
    ...
    // mu and sigma are ONLY required for the Maxwell solver
    if (!m_do_electrostatic) {
    ...
    const amrex::IntVect& Ez_stag)
{
    ...
    // In the Maxwell solver, `epsilon` is used in the denominator. Therefore, it needs to be strictly positive.
    // Electrostatic solver requires an SPD matrix. Therefore, epsilon needs to be strictly positive???
    ...
    const amrex::IntVect sigma_stag = m_sigma_mf ? m_sigma_mf->ixType().toIntVect() : amrex::IntVect(0);
    const amrex::IntVect mu_stag = m_mu_mf ? m_mu_mf->ixType().toIntVect() : amrex::IntVect(0);
