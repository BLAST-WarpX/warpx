...
void InjectorMomentum::clear () // NOLINT(readability-make-member-function-const)
{
    ...
    inj_mom_mean_file_data.reset();
    inj_mom_mean_file_omp.clear();
#endif
}
    ...
    std::function<amrex::Real(amrex::Real)> const& get_zlab)
{
    ...
    if (distributed()) {
        ...
        inj_mom_mean_file_data = std::unique_ptr<void, amrex::DataDeleter>(
            amrex::The_Cpu_Arena()->alloc(sizeof(InjectorMomentum) * nthreads),
            amrex::DataDeleter{amrex::The_Cpu_Arena()});
        auto* p = reinterpret_cast<InjectorMomentum*>(inj_mom_mean_file_data.get());
        inj_mom_mean_file_omp.clear();
        for (int tid = 0; tid < nthreads; ++tid) {
            inj_mom_mean_file_omp.push_back(p++);
        }
        for (auto* q : inj_mom_mean_file_omp) {
            std::memcpy(static_cast<void*>(q), static_cast<void const*>(this), sizeof(InjectorMomentum));
        }
    }
#endif
}
...
#if defined(AMREX_USE_OMP) && !defined(AMREX_USE_GPU)
    if (inj_mom_mean_file_data) {
        auto* my_inj_mom = inj_mom_mean_file_omp[amrex::OpenMP::get_thread_num()];
