...
MultiDiagnostics::MultiDiagnostics (WarpX* warpx)
{
    ReadParameters();
    ...
    for (int i=0; i<ndiags; i++){
        if ( diags_types[i] == DiagTypes::Full || diags_types[i] == DiagTypes::TimeAveraged ){
            alldiags[i] = std::make_unique<FullDiagnostics>(warpx, i, diags_names[i], diags_types[i]);
        } else if ( diags_types[i] == DiagTypes::BackTransformed ){
            alldiags[i] = std::make_unique<BTDiagnostics>(warpx, i, diags_names[i], diags_types[i]);
        } else if ( diags_types[i] == DiagTypes::BoundaryScraping ){
            alldiags[i] = std::make_unique<BoundaryScrapingDiagnostics>(warpx, i, diags_names[i], diags_types[i]);
        } else {
