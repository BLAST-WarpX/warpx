        ...
        for ( int icomp=0; icomp<ncomp; icomp++ ) {
            ...
            for( amrex::MFIter mfi(mf[lev]); mfi.isValid(); ++mfi )
            {
                ...
                if (fab.arena()->isManaged() || fab.arena()->isDevice()) {
                    if (hasADIOS)
                    {
                        WARPX_PROFILE("WarpXOpenPMDPlot::WriteOpenPMDFields::D2H_Span()", ablastr::profiler::OmitUnlessDeviceSynced);
                    ...
                    {
                        WARPX_PROFILE("WarpXOpenPMDPlot::WriteOpenPMDFields::D2H()", ablastr::profiler::OmitUnlessDeviceSynced);
                        amrex::BaseFab<amrex::Real> foo(local_box, 1, amrex::The_Pinned_Arena());
                        std::shared_ptr<amrex::Real> data_pinned(foo.release());
                ...
                { // CPU
                  if (hasADIOS)
                    {
                      WARPX_PROFILE("WarpXOpenPMDPlot::WriteOpenPMDFields::CPU_span()", ablastr::profiler::OmitUnlessDeviceSynced);
                      auto dynamicMemoryView = mesh_comp.storeChunk<amrex::Real>(
                           chunk_offset, chunk_size,
                           [&local_box](size_t /* size */) {
                    ...
                    {
                       WARPX_PROFILE("WarpXOpenPMDPlot::WriteOpenPMDFields::CPU_mesh()", ablastr::profiler::OmitUnlessDeviceSynced);
