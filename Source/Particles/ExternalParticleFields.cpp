...
void ExternalParticleFields::ReadParameters()
{
  ...
  // helper function to read in all the meta data for an external field from file
  auto read_named_spec = [&](const std::string& fname, bool isE) -> ParticleFieldMetaData {
    ...
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
        !md.path.empty(),
        "External particle field '" + fname +
        "' requires 'particles." + fname +
        ".read_fields_from_path = <file>' to be set.");

    // The mathematical expression for the time dependency read_fields_E_dependency(t)
    // can be provided in the input file. If not provided, it defaults to '1.0'
    md.time_function = "1.0";
    const std::string tf_key = isE ? (fname + ".read_fields_E_dependency(t)")
  ...
  if (e_init == "read_from_file") {
    ...
    if (m_E_field_metadata.empty()) {
        ...
        // no E field names provided but read_from_file requested, force to read path
        ...
        m_E_field_metadata.emplace_back(std::move(md));
            ...
            // Provide warning in case syntax for single field is used, although it is initialized
            // via name list
            if (pp_particles.query("read_fields_from_path", single_field_path)){
                ablastr::warn_manager::WMRecordWarning("read_fields_from_path",
                    "particles.read_fields_from_path is ignored when particles.B_ext_particle_fields is used.",
                    ablastr::warn_manager::WarnPriority::low);
            }
            if (pp_particles.query("read_fields_E_dependency(t)", single_field_path)){
                ablastr::warn_manager::WMRecordWarning("read_fields_E_dependency(t)",
                    "particles.read_fields_E_dependency(t) is ignored when particles.E_ext_particle_fields is used.",
                    ablastr::warn_manager::WarnPriority::low);
            }
        }
    ...
    if (b_init == "read_from_file") {
        ...
        if (m_B_field_metadata.empty()) {
            ...
            // no B field names provided but read_from_file requested, force to read path
            pp_particles.get("read_fields_from_path", single_field_path);
            ...
            if (pp_particles.query("read_fields_B_dependency(t)", single_field_path)){
                ablastr::warn_manager::WMRecordWarning("read_fields_B_dependency(t)",
                    "particles.read_fields_B_dependency(t) is ignored when particles.B_ext_particle_fields is used.",
                    ablastr::warn_manager::WarnPriority::low);
            }
        }
    ...
    // since we add both the multi-field read or the single field read, the size can simply be used
