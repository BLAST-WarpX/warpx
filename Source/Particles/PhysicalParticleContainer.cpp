...
#pragma omp parallel
            ...
            if (! do_not_push)
            {
                ...
                if (push_type == PushType::Explicit) {
                    PushPX(pti, exfab, eyfab, ezfab,
                           ...
                           0, np_to_push, lev, gather_lev, dt, ScaleFields(false), subcycling_stage);
                ...
                if (np_gather < np)
                {
                    ...
                    if (push_type == PushType::Explicit) {
                        PushPX(pti, cexfab, ceyfab, cezfab,
                               ...
                               lev, lev-1, dt, ScaleFields(false), subcycling_stage);
                    } else if (push_type == PushType::Implicit) {
    ...
    // coarse level.
    if (do_splitting && (subcycling_stage == SubcyclingStage::SecondHalf || subcycling_stage == SubcyclingStage::None) ){
        SplitParticles(lev);
    }
}
                                   ...
                                   SubcyclingStage subcycling_stage)
{
    ...
    const int do_copy = (m_do_back_transformed_particles && (subcycling_stage!=SubcyclingStage::SecondHalf) );
