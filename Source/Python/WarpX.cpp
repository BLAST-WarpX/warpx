...
namespace detail
{
    ...
    auto get_or_throw (std::string const & prefix, std::string const & name)
    {
        ...
        // has_name = amrex::ParmParse(prefix).queryarr(name, value);
        if constexpr (std::is_same_v<V, bool> || std::is_same_v<V, std::string>) {
            has_name = amrex::ParmParse(prefix).query(name, value);
        }
        else {
            has_name = amrex::ParmParse(prefix).queryWithParser(name, value);
        }
