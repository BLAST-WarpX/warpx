...
#else
// Have not experimented with good tilesize here because expect use case to be
// low
...
isAnyBoundaryPML (
    ...
    constexpr auto is_pml = [] (const FieldBoundaryType fbt) {
        return (fbt == FieldBoundaryType::PML);
    };
    const auto is_any_pml =
        std::any_of(field_boundary_lo.begin(), field_boundary_lo.end(),
                    is_pml) ||
 ...
 */
void
SetDotMask (std::unique_ptr<amrex::iMultiFab>& field_dotMask,
            ablastr::fields::ConstScalarField const& field,
            amrex::Periodicity const& periodicity)

{
    // Define the dot mask for this field_type needed to properly compute
    // dotProduct() for field values that have shared locations on different MPI
    // ranks
    if (field_dotMask != nullptr) {
        return;
    }

    const auto& this_ba = field->boxArray();
    const auto tmp = amrex::MultiFab{this_ba, field->DistributionMap(), 1, 0,
...
WarpX::MakeWarpX () {
    ...
    ReadMovingWindowParameters(do_moving_window, start_moving_window_step,
                               end_moving_window_step, moving_window_dir,
                               moving_window_v);
...
WarpX::WarpX () {
    ...
    if (do_moving_window) {
        ...
        for (int i = 0; i < n_containers; i++) {
            ...
            // elements of current_injection_position easier elsewhere in the
            // code.
            if (moving_window_v > 0._rt) {
                // Inject particles continuously from the right end of the box
                pc.m_current_injection_position =
                    geom[0].ProbHi(moving_window_dir);
    ...
    if ((WarpX::electrostatic_solver_id == ElectrostaticSolverAlgo::LabFrame) ||
        ...
        m_electrostatic_solver =
            std::make_unique<LabFrameExplicitES>(nlevs_max);
    }
    // Initialize the effective potential electrostatic solver if required
    else if (electrostatic_solver_id ==
             ElectrostaticSolverAlgo::LabFrameEffectivePotential) {
        m_electrostatic_solver =
            std::make_unique<EffectivePotentialES>(nlevs_max);
    } else {
        m_electrostatic_solver =
            std::make_unique<RelativisticExplicitES>(nlevs_max);
    }

    if (WarpX::electromagnetic_solver_id ==
    ...
    if (costs_heuristic_cells_wt <= 0. && costs_heuristic_particles_wt <= 0. &&
        WarpX::load_balance_costs_update_algo ==
            LoadBalanceCostsUpdateAlgo::Heuristic) {
#ifdef AMREX_USE_GPU
        if (WarpX::electromagnetic_solver_id ==
...
WarpX::ReadParameters () {
...

    {
        ...
        pp_algo.query_enum_sloppy("maxwell_solver", electromagnetic_solver_id,
                                  "-_");
        if (electromagnetic_solver_id == ElectromagneticSolverAlgo::ECT &&
            !EB::enabled()) {
            throw std::runtime_error("ECP Solver requires to enable embedded "
        ...
        if (electromagnetic_solver_id == ElectromagneticSolverAlgo::PSATD) {
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                Geom(0).ProbLo(0) == 0.,
                "Lower bound of radial coordinate (prob_lo[0]) with RZ PSATD "
                "solver must be zero");
        } else {
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                Geom(0).ProbLo(0) >= 0.,
                "Lower bound of radial coordinate (prob_lo[0]) with RZ FDTD "
                "solver must be non-negative");
        }
#endif
...

    {
        ...
        utils::parser::queryArrWithParser(pp_warpx, "numprocs", numprocs_in, 0,
                                          AMREX_SPACEDIM);
        ...
        if (not numprocs_in.empty()) {
            ...
            if (electromagnetic_solver_id == ElectromagneticSolverAlgo::PSATD) {
                WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                    numprocs_in[0] == 1,
                    "Domain decomposition in RZ with spectral solvers works "
                    "only along z direction");
            }
#endif
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                numprocs_in.size() == AMREX_SPACEDIM,
                "warpx.numprocs, if specified, must have AMREX_SPACEDIM "
                "numbers");
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                ParallelDescriptor::NProcs() == AMREX_D_TERM(numprocs_in[0],
                                                             *numprocs_in[1],
                                                             *numprocs_in[2]),
                "warpx.numprocs, if specified, its product must be equal to "
        ...
        for (const std::string& str : signals_in) {
            ...
            SignalHandling::signal_conf_requests
                [SignalHandling::SIGNAL_REQUESTS_BREAK][sig] = true;
        }
        signals_in.clear();
#else
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            signals_in.empty(), "Signal handling requested in input, but is "
        ...
        pp_warpx.query("write_diagnostics_on_restart",
        ...
        for (const std::string& str : signals_in) {
            const int sig = SignalHandling::parseSignalNameToNumber(str);
            SignalHandling::signal_conf_requests
                [SignalHandling::SIGNAL_REQUESTS_CHECKPOINT][sig] = true;
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                have_checkpoint_diagnostic,
                "Signal handling was requested to checkpoint, but no "
                "checkpoint diagnostic is configured");
        }
#else
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            signals_in.empty(), "Signal handling requested in input, but is "
        ...
        if (random_seed != "default") {
            ...
            if (random_seed == "random") {
                ...
                WARPX_ABORT_WITH_MESSAGE(
                    "warpx.random_seed must be \"default\", \"random\" or an "
                    "integer > 0.");
            }
        }
        ...
        if (do_multi_J) {
            utils::parser::getWithParser(pp_warpx, "do_multi_J_n_depositions",
        ...
        pp_warpx.queryarr("override_sync_intervals",
                          override_sync_intervals_string_vec);
        override_sync_intervals =
            utils::parser::IntervalsParser(override_sync_intervals_string_vec);

        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            m_do_subcycling != 1 || max_level <= 1,
            "Subcycling method 1 only works for 2 levels.");

        ReadBoostedFrameParameters(gamma_boost, beta_boost, boost_direction);

        // queryWithParser returns 1 if argument zmax_plasma_to_compute_max_step
        // is specified by the user, 0 otherwise.
        if (auto temp = 0.0_rt; utils::parser::queryWithParser(
                pp_warpx, "zmax_plasma_to_compute_max_step", temp)) {
            m_zmax_plasma_to_compute_max_step = temp;
        }

        pp_warpx.query("compute_max_step_from_btd", compute_max_step_from_btd);

        if (do_moving_window) {
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                Geom(0).isPeriodic(moving_window_dir) == 0,
                "The problem must be non-periodic in the moving window "
                "direction");
        ...
        if (m_p_ext_field_params->B_ext_grid_type ==
            ...
            m_p_ext_field_params->E_ext_grid_type ==
                ExternalFieldType::read_from_file) {
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                max_level == 0, "External field reading is not implemented for "
                                "more than one level");
        }
        ...
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            (electrostatic_solver_id !=
                 ElectrostaticSolverAlgo::LabFrameElectroMagnetostatic ||
             poisson_solver_id != PoissonSolverAlgo::IntegratedGreenFunction),
            "The FFT Poisson solver is not implemented in "
            "labframe-electromagnetostatic mode yet.");

        [[maybe_unused]] bool const eb_enabled = EB::enabled();
#if !defined(AMREX_USE_EB)
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            !eb_enabled, "Embedded boundaries are requested via "
                         "warpx.eb_enabled but were not compiled!");
#endif

#ifdef WARPX_DIM_RZ
        const ParmParse pp_boundary("boundary");
        pp_boundary.query("verboncoeur_axis_correction",
        ...
        // Filter defaults to true for the explicit scheme, and false for the
        // implicit schemes
        if (evolve_scheme != EvolveScheme::Explicit) {
            use_filter = false;
        }

        // Filter currently not working with FDTD solver in RZ geometry: turn
        // OFF by default (see https://github.com/ECP-WarpX/WarpX/issues/1943)
#ifdef WARPX_DIM_RZ
        if (WarpX::electromagnetic_solver_id !=
            ElectromagneticSolverAlgo::PSATD) {
            WarpX::use_filter = false;
        }
#endif
        ...
        utils::parser::queryArrWithParser(pp_warpx, "filter_npass_each_dir",
                                          parse_filter_npass_each_dir, 0,
                                          AMREX_SPACEDIM);
        ...
        // TODO When k-space filtering will be implemented also for Cartesian
        // geometries, this code block will have to be applied in all cases
        ...
        if (WarpX::electromagnetic_solver_id ==
            ...
            if (WarpX::electromagnetic_solver_id !=
                ...
                // along R (see https://github.com/ECP-WarpX/WarpX/issues/1943)
                WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                    !use_filter || filter_npass_each_dir[0] == 0,
                    "In RZ geometry with FDTD, filtering can only be applied "
                    "along z. This can be controlled by setting "
                    "warpx.filter_npass_each_dir");
            } else {
                if (use_filter && filter_npass_each_dir[0] > 0) {
                    ablastr::warn_manager::WMRecordWarning(
                        "HybridPIC ElectromagneticSolver",
                        "Radial Filtering in RZ is not currently using radial "
                        "geometric weighting to conserve charge. Use at your "
                        "own risk.",
        ...
        utils::parser::queryWithParser(pp_warpx, "num_mirrors", m_num_mirrors);
        if (m_num_mirrors > 0) {
            m_mirror_z.resize(m_num_mirrors);
            utils::parser::getArrWithParser(pp_warpx, "mirror_z", m_mirror_z, 0,
                                            m_num_mirrors);
            m_mirror_z_width.resize(m_num_mirrors);
            utils::parser::getArrWithParser(pp_warpx, "mirror_z_width",
                                            m_mirror_z_width, 0, m_num_mirrors);
            m_mirror_z_npoints.resize(m_num_mirrors);
            utils::parser::getArrWithParser(pp_warpx, "mirror_z_npoints",
                                            m_mirror_z_npoints, 0,
                                            m_num_mirrors);
        }
        ...
        if (do_single_precision_comms) {
            ...
            ablastr::warn_manager::WMRecordWarning(
                "comms",
                "Overwrote warpx.do_single_precision_comms to be 0, since "
                "WarpX was built in single precision.",
                ablastr::warn_manager::WarnPriority::low);
        }
#endif
        pp_warpx.query("do_shared_mem_charge_deposition",
                       do_shared_mem_charge_deposition);
        pp_warpx.query("do_shared_mem_current_deposition",
                       do_shared_mem_current_deposition);
#if !(defined(AMREX_USE_HIP) || defined(AMREX_USE_CUDA))
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            !do_shared_mem_current_deposition,
            "requested shared memory for current deposition, but shared memory "
            "is only available for CUDA or HIP");
#endif
        ...
        const bool shared_tilesize_is_specified =
            utils::parser::queryArrWithParser(pp_warpx, "shared_tilesize",
                                              vect_shared_tilesize, 0,
                                              AMREX_SPACEDIM);
        ...
        pp_warpx.query("serialize_initial_conditions",
        ...
        utils::parser::queryWithParser(pp_warpx, "n_field_gather_buffer",
                                       n_field_gather_buffer);
        utils::parser::queryWithParser(pp_warpx, "n_current_deposition_buffer",
                                       n_current_deposition_buffer);

        // Default value for the quantum parameter used in Maxwell’s QED
        // equations
        m_quantum_xi_c2 = PhysConst::xi_c2;

        amrex::Real quantum_xi_tmp;
        const auto quantum_xi_is_specified = utils::parser::queryWithParser(
            pp_warpx, "quantum_xi", quantum_xi_tmp);
        if (quantum_xi_is_specified) {
            double const quantum_xi = quantum_xi_tmp;
            m_quantum_xi_c2 = static_cast<amrex::Real>(
                quantum_xi * PhysConst::c * PhysConst::c);
        }

        const auto at_least_one_boundary_is_pml =
            (std::any_of(WarpX::field_boundary_lo.begin(),
                         WarpX::field_boundary_lo.end(),
                         [] (const auto& cc) {
                             return cc == FieldBoundaryType::PML;
                         }) ||
             std::any_of(WarpX::field_boundary_hi.begin(),
                         WarpX::field_boundary_hi.end(), [] (const auto& cc) {
                             return cc == FieldBoundaryType::PML;
                         }));
        ...
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            !(at_least_one_boundary_is_pml &&
              at_least_one_boundary_is_silver_mueller),
            "PML and Silver-Mueller boundary conditions cannot be activated at "
            "the same time.");

        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            (!at_least_one_boundary_is_silver_mueller) ||
                (electromagnetic_solver_id == ElectromagneticSolverAlgo::Yee),
            "The Silver-Mueller boundary condition can only be used with the "
            "Yee solver.");
        ...
        utils::parser::queryWithParser(pp_warpx, "v_particle_pml",
                                       v_particle_pml);
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            0._rt < v_particle_pml && v_particle_pml <= 1._rt,
            "Input value for the velocity warpx.v_particle_pml of the "
            "macroparticle must be in (0,1] (in units of c).");
        // Scale by the speed of light
        v_particle_pml = v_particle_pml * PhysConst::c;

        // Default values of WarpX::do_pml_dive_cleaning and
        // WarpX::do_pml_divb_cleaning: true for Cartesian PSATD solver, false
        // otherwise
        ...
        // If WarpX::do_dive_cleaning = true, set also
        // WarpX::do_pml_dive_cleaning = true (possibly overwritten by users in
        // the input file, see query below)
        if (do_dive_cleaning) {
            do_pml_dive_cleaning = true;
        }

        // If WarpX::do_divb_cleaning = true, set also
        // WarpX::do_pml_divb_cleaning = true (possibly overwritten by users in
        // the input file, see query below)
        // TODO Implement div(B) cleaning in PML with FDTD and remove second if
        // condition
        if (do_divb_cleaning &&
        ...
        if (electromagnetic_solver_id != ElectromagneticSolverAlgo::PSATD) {
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                do_pml_divb_cleaning == false,
                "warpx.do_pml_divb_cleaning = true not implemented for FDTD "
                "solver");
        }
...
#ifdef WARPX_DIM_RZ
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            ::isAnyBoundaryPML(field_boundary_lo, field_boundary_hi) == false ||
                electromagnetic_solver_id == ElectromagneticSolverAlgo::PSATD,
            "PML are not implemented in RZ geometry with FDTD; please set a "
            "different boundary condition using boundary.field_lo and "
            "boundary.field_hi.");
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            field_boundary_lo[1] != FieldBoundaryType::PML &&
                field_boundary_hi[1] != FieldBoundaryType::PML,
            "PML are not implemented in RZ geometry along z; please set a "
            "different boundary condition using boundary.field_lo and "
            "boundary.field_hi.");
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            (do_pml_dive_cleaning == false && do_pml_divb_cleaning == false),
            "do_pml_dive_cleaning and do_pml_divb_cleaning are not implemented "
            "in RZ geometry.");
#endif

        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            (do_pml_j_damping == 0) || (do_pml_in_domain == 1),
            "J-damping can only be done when PML are inside simulation domain "
            "(do_pml_in_domain=1)");

        pp_warpx.query("synchronize_velocity_for_diagnostics",
...

        {
            ...
            utils::parser::queryWithParser(pp_warpx, "mffile_nstreams",
                                           mffile_nstreams);
            VisMF::SetMFFileInStreams(mffile_nstreams);
            utils::parser::queryWithParser(pp_warpx, "field_io_nfiles",
                                           field_io_nfiles);
        ...
        if (maxLevel() > 0) {
            Vector<Real> lo, hi;
            const bool fine_tag_lo_specified =
                utils::parser::queryArrWithParser(pp_warpx, "fine_tag_lo", lo);
            const bool fine_tag_hi_specified =
                utils::parser::queryArrWithParser(pp_warpx, "fine_tag_hi", hi);
            std::string ref_patch_function;
            const bool parser_specified =
                pp_warpx.query("ref_patch_function(x,y,z)", ref_patch_function);
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                ((fine_tag_lo_specified && fine_tag_hi_specified) ||
                 parser_specified),
            ...
            if ((fine_tag_lo_specified && fine_tag_hi_specified) &&
                parser_specified) {
                ablastr::warn_manager::WMRecordWarning(
                    "Refined patch", "Both fine_tag_lo,fine_tag_hi\
            ...
            if (fine_tag_lo_specified && fine_tag_hi_specified) {
                ...
                utils::parser::Store_parserString(
                    pp_warpx, "ref_patch_function(x,y,z)", ref_patch_function);
                ref_patch_parser =
        ...
        utils::parser::queryWithParser(pp_warpx, "n_rz_azimuthal_modes",
                                       n_rz_azimuthal_modes);
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            n_rz_azimuthal_modes > 0,
            "The number of azimuthal modes (n_rz_azimuthal_modes) must be at "
            "least 1");
#endif
        ...
        // Check whether fluid species will be used
        {
            ...
            if (do_fluid_species) {
                WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                    max_level <= 1, "Fluid species cannot currently be used "
                                    "with mesh refinement.");
                WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                    electrostatic_solver_id !=
                        ElectrostaticSolverAlgo::Relativistic,
                    "Fluid species cannot currently be used with the "
                    "relativistic electrostatic solver.");
#ifdef WARPX_DIM_RZ
                WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                    n_rz_azimuthal_modes <= 1,
                    "Fluid species cannot be used with more than 1 azimuthal "
                    "mode.");
#endif
            }
        }
        ...
        // Update default to external projection divb cleaner if external fields
        // are loaded, the grids are staggered, and the solver is compatible
        // with the cleaner
        if (!do_divb_cleaning &&
            m_p_ext_field_params->B_ext_grid_type !=
                ExternalFieldType::default_zero &&
            m_p_ext_field_params->B_ext_grid_type !=
                ExternalFieldType::constant &&
            grid_type != GridType::Collocated &&
            (WarpX::electromagnetic_solver_id ==
                 ElectromagneticSolverAlgo::Yee ||
             WarpX::electromagnetic_solver_id ==
                 ElectromagneticSolverAlgo::HybridPIC ||
             ((WarpX::electrostatic_solver_id ==
        ...
        pp_warpx.query("do_divb_cleaning_external",
        ...
        if (do_current_centering) {
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                grid_type == GridType::Hybrid,
                "warpx.do_current_centering=1 can be used only with "
                "warpx.grid_type=hybrid");

            utils::parser::queryWithParser(pp_warpx, "current_centering_nox",
                                           current_centering_nox);
            utils::parser::queryWithParser(pp_warpx, "current_centering_noy",
                                           current_centering_noy);
            utils::parser::queryWithParser(pp_warpx, "current_centering_noz",
            ...
            AllocateCenteringCoefficients(
                ...
                current_centering_noz, grid_type);
        }

        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            maxLevel() == 0 || !do_current_centering,
            "Finite-order centering of currents is not implemented with mesh "
            "refinement");
    }

    {
        ...
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            electromagnetic_solver_id != ElectromagneticSolverAlgo::PSATD,
            "algo.maxwell_solver = psatd is not supported because WarpX was "
            "built without spectral solvers");
#endif
        ...
        // note: current_deposition must be set after maxwell_solver
        // (electromagnetic_solver_id) or
        //       do_electrostatic (electrostatic_solver_id) are already
        //       determined, because its default depends on the solver selection
        if (electromagnetic_solver_id == ElectromagneticSolverAlgo::PSATD ||
            electromagnetic_solver_id == ElectromagneticSolverAlgo::HybridPIC ||
            electrostatic_solver_id != ElectrostaticSolverAlgo::None) {
            current_deposition_algo = CurrentDepositionAlgo::Direct;
        }
        pp_algo.query_enum_sloppy("current_deposition", current_deposition_algo,
                                  "-_");
        pp_algo.query_enum_sloppy("charge_deposition", charge_deposition_algo,
                                  "-_");
        pp_algo.query_enum_sloppy("particle_pusher", particle_pusher_algo,
                                  "-_");
        ...
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            ...
            "Current centering (nodal deposition) cannot be used with "
            "Esirkepov deposition."
            "Please set warpx.do_current_centering = 0 or "
        ...
        // "default" if not found (default defined in
        // Utils/WarpXAlgorithmSelection.cpp)
        pp_algo.query_enum_sloppy("field_gathering", field_gathering_algo,
                                  "-_");

        // Set default field gathering algorithm for hybrid grids
        // (momentum-conserving)
        std::string tmp_algo;
        ...
        // - field_gathering_algo read above and set to user-defined value
            ...
            if (grid_type == GridType::Hybrid) {
                WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                    field_gathering_algo == GatheringAlgo::MomentumConserving,
                    "Hybrid grid (warpx.grid_type=hybrid) should be used only "
                    "with "
        ...
        if (evolve_scheme == EvolveScheme::SemiImplicitEM ||
            ...
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                current_deposition_algo == CurrentDepositionAlgo::Esirkepov ||
                    ...
                    current_deposition_algo == CurrentDepositionAlgo::Direct,
                "Only Esirkepov, Villasenor, or Direct current deposition "
                "supported with the implicit and semi-implicit schemes");

            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                electromagnetic_solver_id == ElectromagneticSolverAlgo::Yee ||
                    electromagnetic_solver_id ==
                        ElectromagneticSolverAlgo::CKC ||
                    electromagnetic_solver_id ==
                        ElectromagneticSolverAlgo::PSATD,
                "Only the Yee, CKC, and PSATD EM solvers are supported with "
                "the implicit and semi-implicit schemes");
            ...
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                ...
                "with the implicit and semi-implicit schemes");

            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                field_gathering_algo != GatheringAlgo::MomentumConserving,
                "With implicit and semi-implicit schemes, the momentum "
                "conserving field gather is not supported as it would not "
                "conserve energy");
        }
        if (evolve_scheme == EvolveScheme::StrangImplicitSpectralEM) {
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                electromagnetic_solver_id == ElectromagneticSolverAlgo::PSATD,
                "With the strang_implicit_spectral_em evolve scheme, the "
                "algo.maxwell_solver must be psatd");
        }

        // Load balancing parameters
        std::vector<std::string> load_balance_intervals_string_vec = {"0"};
        pp_algo.queryarr("load_balance_intervals",
                         load_balance_intervals_string_vec);
        load_balance_intervals =
            utils::parser::IntervalsParser(load_balance_intervals_string_vec);
        ...
        if (!load_balance_with_sfc) {
            pp_algo.query("load_balance_knapsack_factor",
                          load_balance_knapsack_factor);
        }
        utils::parser::queryWithParser(
            pp_algo, "load_balance_efficiency_ratio_threshold",
            load_balance_efficiency_ratio_threshold);
        pp_algo.query_enum_sloppy("load_balance_costs_update",
                                  load_balance_costs_update_algo, "-_");
        if (WarpX::load_balance_costs_update_algo ==
            LoadBalanceCostsUpdateAlgo::Heuristic) {
            utils::parser::queryWithParser(pp_algo, "costs_heuristic_cells_wt",
                                           costs_heuristic_cells_wt);
            utils::parser::queryWithParser(pp_algo,
        ...
        if (!lasers_names.empty() && n_rz_azimuthal_modes < 2) {
            ablastr::warn_manager::WMRecordWarning(
                "Laser",
                "Laser pulse representation in RZ requires at least 2 "
                "azimuthal modes",
        ...
        if (!species_names.empty() || !lasers_names.empty()) {
            if (utils::parser::queryWithParser(pp_algo, "particle_shape",
                                               particle_shape)) {
                WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            ...
            if ((maxLevel() > 0) && (particle_shape > 1) &&
                (do_pml_j_damping == 1)) {
                ablastr::warn_manager::WMRecordWarning(
                    "Particles", "When algo.particle_shape > 1,"
                                 "some numerical artifact will be present at "
                                 "the interface between coarse and fine patch."
                                 "We recommend setting algo.particle_shape = 1 "
                                 "in order to avoid this issue");
            }

            // default sort interval for particles if species or lasers vector
            // is not empty
#ifdef AMREX_USE_GPU
        ...
        sort_intervals =
            utils::parser::IntervalsParser(sort_intervals_string_vec);

        Vector<int> vect_sort_bin_size(AMREX_SPACEDIM, 1);
        const bool sort_bin_size_is_specified =
            utils::parser::queryArrWithParser(pp_warpx, "sort_bin_size",
                                              vect_sort_bin_size, 0,
                                              AMREX_SPACEDIM);
        ...
        pp_warpx.query("sort_particles_for_deposition",
                       sort_particles_for_deposition);
        Vector<int> vect_sort_idx_type(AMREX_SPACEDIM, 0);
        const bool sort_idx_type_is_specified =
            utils::parser::queryArrWithParser(pp_warpx, "sort_idx_type",
                                              vect_sort_idx_type, 0,
                                              AMREX_SPACEDIM);
...

    {
        ...
        if ((WarpX::field_gathering_algo == GatheringAlgo::MomentumConserving &&
            ...
            WarpX::electrostatic_solver_id ==
                ElectrostaticSolverAlgo::LabFrameElectroMagnetostatic) {
            utils::parser::queryWithParser(pp_warpx, "field_centering_nox",
                                           field_centering_nox);
            utils::parser::queryWithParser(pp_warpx, "field_centering_noy",
                                           field_centering_noy);
            utils::parser::queryWithParser(pp_warpx, "field_centering_noz",
                                           field_centering_noz);
            ...
            AllocateCenteringCoefficients(
                ...
                field_centering_noy, field_centering_noz, grid_type);
        }

        // Finite-order centering is not implemented with mesh refinement
        // (note that when warpx.grid_type=collocated, finite-order centering is
        // not used anyways)
        if (maxLevel() > 0 && WarpX::grid_type != GridType::Collocated) {
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                field_centering_nox == 2 && field_centering_noy == 2 &&
                    field_centering_noz == 2,
                "High-order centering of fields (order > 2) is not implemented "
                "with mesh refinement");
        }
    }
    ...
    if (electromagnetic_solver_id == ElectromagneticSolverAlgo::PSATD) {
        ...
        if (!fft_periodic_single_box) {
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                nox_fft > 0, "PSATD order must be finite unless "
                             "psatd.periodic_single_box_fft is used");
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                noy_fft > 0, "PSATD order must be finite unless "
                             "psatd.periodic_single_box_fft is used");
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                noz_fft > 0, "PSATD order must be finite unless "
                             "psatd.periodic_single_box_fft is used");
        }
        ...
        // Integers that correspond to the time dependency of J (constant,
        // linear) and rho (linear, quadratic) for the PSATD algorithm
        pp_psatd.query_enum_sloppy("J_in_time", J_in_time, "-_");
        pp_psatd.query_enum_sloppy("rho_in_time", rho_in_time, "-_");

        if (m_psatd_solution_type != PSATDSolutionType::FirstOrder ||
            !do_multi_J) {
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                rho_in_time == RhoInTime::Linear,
                "psatd.rho_in_time=constant not yet implemented, "
                "except for psatd.solution_type=first-order and "
                "warpx.do_multi_J=1");
        }
        ...
        // are used
        if (WarpX::current_deposition_algo ==
        ...
        if (WarpX::current_deposition_algo == CurrentDepositionAlgo::Vay) {
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                ...
                "Option algo.current_deposition=vay must be used with "
        ...
        if (current_deposition_algo == CurrentDepositionAlgo::Vay) {
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                !current_correction,
                "Options algo.current_deposition=vay and "
                "psatd.current_correction=1 cannot be combined together.");
        }

        // Auxiliary: boosted_frame = true if WarpX::gamma_boost is set in the
        // inputs
        const amrex::ParmParse pp_warpx("warpx");
        ...
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            ...
            "psatd.use_default_v_galilean = 1 can be used only if "
            "WarpX::gamma_boost is also set");

        if (use_default_v_galilean && boosted_frame) {
            m_v_galilean[2] =
                -std::sqrt(1._rt - 1._rt / (gamma_boost * gamma_boost));
        } else {
            utils::parser::queryArrWithParser(pp_psatd, "v_galilean",
                                              m_v_galilean, 0, 3);
        }
        ...
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            ...
            "psatd.use_default_v_comoving = 1 can be used only if "
            "WarpX::gamma_boost is also set");
        ...
        if (use_default_v_comoving && boosted_frame) {
            ...
            utils::parser::queryArrWithParser(pp_psatd, "v_comoving",
                                              m_v_comoving, 0, 3);
        }

        // Scale the Galilean/comoving velocity by the speed of light
        for (auto& vv : m_v_galilean) {
            vv *= PhysConst::c;
        }
        for (auto& vv : m_v_comoving) {
            vv *= PhysConst::c;
        }
        ...
        if (current_deposition_algo == CurrentDepositionAlgo::Esirkepov ||
            ...
            // The comoving PSATD algorithm is not implemented nor tested with
            // Esirkepov current deposition
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                v_comoving_is_zero,
                "charge-conserving current depositions (Esirkepov and "
                "Villasenor) cannot be used with the comoving PSATD algorithm");

            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                v_galilean_is_zero,
                "charge-conserving current depositions (Esirkepov and "
                "Villasenor) cannot be used with the Galilean algorithm.");
        }
...
#else
        if (m_v_galilean[0] == 0. && m_v_galilean[1] == 0. &&
            m_v_galilean[2] == 0. && m_v_comoving[0] == 0. &&
            m_v_comoving[1] == 0. && m_v_comoving[2] == 0.) {
        ...
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            ...
            "warpx.do_dive_cleaning = 1 not implemented with "
        ...
        if (J_in_time == JInTime::Linear) {
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                update_with_rho, "psatd.update_with_rho must be set to 1 when "
            ...
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                !current_correction, "psatd.current_correction=1 not "
                                     "implemented with psatd.J_in_time=linear");

            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                current_deposition_algo != CurrentDepositionAlgo::Vay,
                "algo.current_deposition=vay not implemented with "
                "psatd.J_in_time=linear");
        }
        ...
        for (int dir = 0; dir < AMREX_SPACEDIM; dir++) {
            if (WarpX::field_boundary_lo[dir] == FieldBoundaryType::Damped ||
                ...
                WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                    WarpX::field_boundary_lo[dir] ==
                        WarpX::field_boundary_hi[dir],
                    "field boundary in both lo and hi must be set to Damped "
                    "for PSATD");
            }
        }
    ...
    // for slice generation //
    {
        ...
        utils::parser::queryArrWithParser(pp_slice, "dom_lo", slice_lo, 0,
                                          AMREX_SPACEDIM);
        utils::parser::queryArrWithParser(pp_slice, "dom_hi", slice_hi, 0,
                                          AMREX_SPACEDIM);
        utils::parser::queryArrWithParser(pp_slice, "coarsening_ratio",
                                          slice_crse_ratio, 0, AMREX_SPACEDIM);
        utils::parser::queryWithParser(pp_slice, "plot_int", slice_plot_int);
...
WarpX::BackwardCompatibility () {
    ...
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
        ...
        "amr.plot_int is not supported anymore. Please use the new syntax for "
        "diagnostics:\n"
        "diagnostics.diags_names = my_diag\n"
        "my_diag.intervals = 10\n"
        "for output every 10 iterations. See documentation for more "
        "information");
    ...
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
        !pp_warpx.query("load_balance_efficiency_ratio_threshold",
                        backward_Real),
        "warpx.load_balance_efficiency_ratio_threshold is not supported "
        "anymore. "
        "Please use the renamed option "
    ...
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
        ...
        "do_pml is not supported anymore. Please use boundary.field_lo and "
        "boundary.field_hi"
    ...
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
        ...
        "Please use the renamed option warpx.serialize_initial_conditions "
        "instead.");

    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
        !pp_warpx.query("do_back_transformed_diagnostics", backward_int),
        "Legacy back-transformed diagnostics are not supported anymore. "
        "Please use the new syntax for back-transformed diagnostics, see "
        "documentation.");
    ...
    if (pp_particles.query("nspecies", nspecies)) {
        ablastr::warn_manager::WMRecordWarning(
            "Species",
            "particles.nspecies is ignored. Just use particles.species_names "
            "please.",
            ablastr::warn_manager::WarnPriority::low);
    }
    ...
    for (const std::string& speciesiter : backward_sp_names) {
        ...
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            !pp_species.queryarr("multiple_particles_vel_x", backward_vel) &&
                !pp_species.queryarr("multiple_particles_vel_y",
                                     backward_vel) &&
    ...
    if (pp_collisions.query("ncollisions", ncollisions)) {
        ablastr::warn_manager::WMRecordWarning(
            "Collisions",
            "collisions.ncollisions is ignored. Just use "
            "particles.collision_names please.",
            ablastr::warn_manager::WarnPriority::low);
    }
    ...
    if (pp_lasers.query("nlasers", nlasers)) {
        ablastr::warn_manager::WMRecordWarning(
            "Laser", "lasers.nlasers is ignored. Just use lasers.names please.",
            ablastr::warn_manager::WarnPriority::low);
    }
}
...
// This is a virtual function.
void
WarpX::MakeNewLevelFromCoarse (int /*lev*/, amrex::Real /*time*/,
...
void
WarpX::AllocLevelData (int lev, const BoxArray& ba,
    ...
    guard_cells.Init(
        ...
        do_moving_window, moving_window_dir, WarpX::nox, nox_fft, noy_fft,
        noz_fft, NCIGodfreyFilter::m_stencil_width, electromagnetic_solver_id,
        maxLevel(), WarpX::m_v_galilean, WarpX::m_v_comoving,
        m_safe_guard_cells, WarpX::do_multi_J, WarpX::fft_do_time_averaging,
        ::isAnyBoundaryPML(field_boundary_lo, field_boundary_hi),
        WarpX::do_pml_in_domain, WarpX::pml_ncell, this->refRatio(), use_filter,
    ...
    if (eb_enabled) {
        ...
        m_field_factory[lev] = amrex::makeEBFabFactory(
            Geom(lev), ba, dm, {max_guard, max_guard, max_guard},
    ...
    AllocLevelMFs(lev, ba, dm, guard_cells.ng_alloc_EB, guard_cells.ng_alloc_J,
                  ...
                  guard_cells.ng_alloc_G, aux_is_nodal);
...
void
WarpX::AllocLevelMFs (int lev, const BoxArray& ba,
                      const DistributionMapping& dm, const IntVect& ngEB,
                      IntVect& ngJ, const IntVect& ngRho, const IntVect& ngF,
                      const IntVect& ngG, const bool aux_is_nodal) {
    ...
    // AMReX convention: x = missing dimension, y = missing dimension, z = only
    // dimension
    ...
    // AMReX convention: x = first dimension, y = missing dimension, z = second
    // dimension
    ...
    // Set global rho nodal flag to know about rho index type when rho MultiFab
    // is not allocated
    ...
    m_fields.alloc_init(FieldType::Bfield_fp, Direction{0}, lev,
                        amrex::convert(ba, Bx_nodal_flag), dm, ncomps, ngEB,
                        0.0_rt);
    m_fields.alloc_init(FieldType::Bfield_fp, Direction{1}, lev,
                        amrex::convert(ba, By_nodal_flag), dm, ncomps, ngEB,
                        0.0_rt);
    m_fields.alloc_init(FieldType::Bfield_fp, Direction{2}, lev,
                        amrex::convert(ba, Bz_nodal_flag), dm, ncomps, ngEB,
                        0.0_rt);

    m_fields.alloc_init(FieldType::Efield_fp, Direction{0}, lev,
                        amrex::convert(ba, Ex_nodal_flag), dm, ncomps, ngEB,
                        0.0_rt);
    m_fields.alloc_init(FieldType::Efield_fp, Direction{1}, lev,
                        amrex::convert(ba, Ey_nodal_flag), dm, ncomps, ngEB,
                        0.0_rt);
    m_fields.alloc_init(FieldType::Efield_fp, Direction{2}, lev,
                        amrex::convert(ba, Ez_nodal_flag), dm, ncomps, ngEB,
                        0.0_rt);

    m_fields.alloc_init(FieldType::current_fp, Direction{0}, lev,
                        amrex::convert(ba, jx_nodal_flag), dm, ncomps, ngJ,
                        0.0_rt);
    m_fields.alloc_init(FieldType::current_fp, Direction{1}, lev,
                        amrex::convert(ba, jy_nodal_flag), dm, ncomps, ngJ,
                        0.0_rt);
    m_fields.alloc_init(FieldType::current_fp, Direction{2}, lev,
                        amrex::convert(ba, jz_nodal_flag), dm, ncomps, ngJ,
                        0.0_rt);

    if (do_current_centering) {
        amrex::BoxArray const& nodal_ba =
            amrex::convert(ba, amrex::IntVect::TheNodeVector());
        m_fields.alloc_init(FieldType::current_fp_nodal, Direction{0}, lev,
                            nodal_ba, dm, ncomps, ngJ, 0.0_rt);
        m_fields.alloc_init(FieldType::current_fp_nodal, Direction{1}, lev,
                            nodal_ba, dm, ncomps, ngJ, 0.0_rt);
        m_fields.alloc_init(FieldType::current_fp_nodal, Direction{2}, lev,
                            nodal_ba, dm, ncomps, ngJ, 0.0_rt);
    }

    if (WarpX::current_deposition_algo == CurrentDepositionAlgo::Vay) {
        m_fields.alloc_init(FieldType::current_fp_vay, Direction{0}, lev,
                            amrex::convert(ba, rho_nodal_flag), dm, ncomps, ngJ,
                            0.0_rt);
        m_fields.alloc_init(FieldType::current_fp_vay, Direction{1}, lev,
                            amrex::convert(ba, rho_nodal_flag), dm, ncomps, ngJ,
                            0.0_rt);
        m_fields.alloc_init(FieldType::current_fp_vay, Direction{2}, lev,
                            amrex::convert(ba, rho_nodal_flag), dm, ncomps, ngJ,
                            0.0_rt);
    }
    ...
    if (electrostatic_solver_id ==
        ElectrostaticSolverAlgo::LabFrameElectroMagnetostatic) {
        m_fields.alloc_init(FieldType::vector_potential_fp_nodal, Direction{0},
                            lev, amrex::convert(ba, rho_nodal_flag), dm, ncomps,
                            ngRho, 0.0_rt);
        m_fields.alloc_init(FieldType::vector_potential_fp_nodal, Direction{1},
                            lev, amrex::convert(ba, rho_nodal_flag), dm, ncomps,
                            ngRho, 0.0_rt);
        m_fields.alloc_init(FieldType::vector_potential_fp_nodal, Direction{2},
                            lev, amrex::convert(ba, rho_nodal_flag), dm, ncomps,
                            ngRho, 0.0_rt);

        // Memory buffers for computing magnetostatic fields
        // Vector Potential A and previous step.  Time buffer needed for
        // computing dA/dt to first order
        m_fields.alloc_init(
            FieldType::vector_potential_grad_buf_e_stag, Direction{0}, lev,
            amrex::convert(ba, Ex_nodal_flag), dm, ncomps, ngEB, 0.0_rt);
        m_fields.alloc_init(
            FieldType::vector_potential_grad_buf_e_stag, Direction{1}, lev,
            amrex::convert(ba, Ey_nodal_flag), dm, ncomps, ngEB, 0.0_rt);
        m_fields.alloc_init(
            FieldType::vector_potential_grad_buf_e_stag, Direction{2}, lev,
            amrex::convert(ba, Ez_nodal_flag), dm, ncomps, ngEB, 0.0_rt);

        m_fields.alloc_init(
            FieldType::vector_potential_grad_buf_b_stag, Direction{0}, lev,
            amrex::convert(ba, Bx_nodal_flag), dm, ncomps, ngEB, 0.0_rt);
        m_fields.alloc_init(
            FieldType::vector_potential_grad_buf_b_stag, Direction{1}, lev,
            amrex::convert(ba, By_nodal_flag), dm, ncomps, ngEB, 0.0_rt);
        m_fields.alloc_init(
            FieldType::vector_potential_grad_buf_b_stag, Direction{2}, lev,
            amrex::convert(ba, Bz_nodal_flag), dm, ncomps, ngEB, 0.0_rt);
    }
    ...
    if (WarpX::electromagnetic_solver_id ==
        ...
        m_hybrid_pic_model->AllocateLevelMFs(
            m_fields, lev, ba, dm, ncomps, ngJ, ngRho, ngEB, jx_nodal_flag,
            jy_nodal_flag, jz_nodal_flag, rho_nodal_flag, Ex_nodal_flag,
            Ey_nodal_flag, Ez_nodal_flag, Bx_nodal_flag, By_nodal_flag,
            Bz_nodal_flag);
    }
    ...
    if (fft_do_time_averaging) {
        m_fields.alloc_init(FieldType::Bfield_avg_fp, Direction{0}, lev,
                            amrex::convert(ba, Bx_nodal_flag), dm, ncomps, ngEB,
                            0.0_rt);
        m_fields.alloc_init(FieldType::Bfield_avg_fp, Direction{1}, lev,
                            amrex::convert(ba, By_nodal_flag), dm, ncomps, ngEB,
                            0.0_rt);
        m_fields.alloc_init(FieldType::Bfield_avg_fp, Direction{2}, lev,
                            amrex::convert(ba, Bz_nodal_flag), dm, ncomps, ngEB,
                            0.0_rt);

        m_fields.alloc_init(FieldType::Efield_avg_fp, Direction{0}, lev,
                            amrex::convert(ba, Ex_nodal_flag), dm, ncomps, ngEB,
                            0.0_rt);
        m_fields.alloc_init(FieldType::Efield_avg_fp, Direction{1}, lev,
                            amrex::convert(ba, Ey_nodal_flag), dm, ncomps, ngEB,
                            0.0_rt);
        m_fields.alloc_init(FieldType::Efield_avg_fp, Direction{2}, lev,
                            amrex::convert(ba, Ez_nodal_flag), dm, ncomps, ngEB,
                            0.0_rt);
    }
    ...
    if (EB::enabled()) {
        ...
        // EB level set
        m_fields.alloc_init(FieldType::distance_to_eb, lev,
                            amrex::convert(ba, IntVect::TheNodeVector()), dm,
                            nc_ls, ng_ls, 0.0_rt);
        // Whether to reduce the particle shape to order 1 when close to the EB
        AllocInitMultiFab(m_eb_reduce_particle_shape[lev],
                          amrex::convert(ba, IntVect::TheCellVector()), dm,
                          ncomps, ngRho, lev, "m_eb_reduce_particle_shape");
        ...
        if (lev == maxLevel()) {
            ...
            if (WarpX::electromagnetic_solver_id !=
                ...
                AllocInitMultiFab(m_eb_update_E[lev][0],
                                  amrex::convert(ba, Ex_nodal_flag), dm, ncomps,
                                  guard_cells.ng_FieldSolver, lev,
                                  "m_eb_update_E[x]");
                AllocInitMultiFab(m_eb_update_E[lev][1],
                                  amrex::convert(ba, Ey_nodal_flag), dm, ncomps,
                                  guard_cells.ng_FieldSolver, lev,
                                  "m_eb_update_E[y]");
                AllocInitMultiFab(m_eb_update_E[lev][2],
                                  amrex::convert(ba, Ez_nodal_flag), dm, ncomps,
                                  guard_cells.ng_FieldSolver, lev,
                                  "m_eb_update_E[z]");

                AllocInitMultiFab(m_eb_update_B[lev][0],
                                  amrex::convert(ba, Bx_nodal_flag), dm, ncomps,
                                  guard_cells.ng_FieldSolver, lev,
                                  "m_eb_update_B[x]");
                AllocInitMultiFab(m_eb_update_B[lev][1],
                                  amrex::convert(ba, By_nodal_flag), dm, ncomps,
                                  guard_cells.ng_FieldSolver, lev,
                                  "m_eb_update_B[y]");
                AllocInitMultiFab(m_eb_update_B[lev][2],
                                  amrex::convert(ba, Bz_nodal_flag), dm, ncomps,
                                  guard_cells.ng_FieldSolver, lev,
                                  "m_eb_update_B[z]");
            }
            if (WarpX::electromagnetic_solver_id ==
                ElectromagneticSolverAlgo::ECT) {

                //! EB: Lengths of the mesh edges
                m_fields.alloc_init(FieldType::edge_lengths, Direction{0}, lev,
                                    amrex::convert(ba, Ex_nodal_flag), dm,
                                    ncomps, guard_cells.ng_FieldSolver, 0.0_rt);
                m_fields.alloc_init(FieldType::edge_lengths, Direction{1}, lev,
                                    amrex::convert(ba, Ey_nodal_flag), dm,
                                    ncomps, guard_cells.ng_FieldSolver, 0.0_rt);
                m_fields.alloc_init(FieldType::edge_lengths, Direction{2}, lev,
                                    amrex::convert(ba, Ez_nodal_flag), dm,
                                    ncomps, guard_cells.ng_FieldSolver, 0.0_rt);

                //! EB: Areas of the mesh faces
                m_fields.alloc_init(FieldType::face_areas, Direction{0}, lev,
                                    amrex::convert(ba, Bx_nodal_flag), dm,
                                    ncomps, guard_cells.ng_FieldSolver, 0.0_rt);
                m_fields.alloc_init(FieldType::face_areas, Direction{1}, lev,
                                    amrex::convert(ba, By_nodal_flag), dm,
                                    ncomps, guard_cells.ng_FieldSolver, 0.0_rt);
                m_fields.alloc_init(FieldType::face_areas, Direction{2}, lev,
                                    amrex::convert(ba, Bz_nodal_flag), dm,
                                    ncomps, guard_cells.ng_FieldSolver, 0.0_rt);

                AllocInitMultiFab(m_flag_info_face[lev][0],
                                  amrex::convert(ba, Bx_nodal_flag), dm, ncomps,
                                  guard_cells.ng_FieldSolver, lev,
                                  "m_flag_info_face[x]");
                AllocInitMultiFab(m_flag_info_face[lev][1],
                                  amrex::convert(ba, By_nodal_flag), dm, ncomps,
                                  guard_cells.ng_FieldSolver, lev,
                                  "m_flag_info_face[y]");
                AllocInitMultiFab(m_flag_info_face[lev][2],
                                  amrex::convert(ba, Bz_nodal_flag), dm, ncomps,
                                  guard_cells.ng_FieldSolver, lev,
                                  "m_flag_info_face[z]");
                AllocInitMultiFab(m_flag_ext_face[lev][0],
                                  amrex::convert(ba, Bx_nodal_flag), dm, ncomps,
                                  guard_cells.ng_FieldSolver, lev,
                                  "m_flag_ext_face[x]");
                AllocInitMultiFab(m_flag_ext_face[lev][1],
                                  amrex::convert(ba, By_nodal_flag), dm, ncomps,
                                  guard_cells.ng_FieldSolver, lev,
                                  "m_flag_ext_face[y]");
                AllocInitMultiFab(m_flag_ext_face[lev][2],
                                  amrex::convert(ba, Bz_nodal_flag), dm, ncomps,
                                  guard_cells.ng_FieldSolver, lev,
                                  "m_flag_ext_face[z]");

                /** EB: area_mod contains the modified areas of the mesh faces,
                 * i.e. if a face is enlarged it contains the area of the
                 * enlarged face This is only used for the ECT solver.*/
                m_fields.alloc_init(FieldType::area_mod, Direction{0}, lev,
                                    amrex::convert(ba, Bx_nodal_flag), dm,
                                    ncomps, guard_cells.ng_FieldSolver, 0.0_rt);
                m_fields.alloc_init(FieldType::area_mod, Direction{1}, lev,
                                    amrex::convert(ba, By_nodal_flag), dm,
                                    ncomps, guard_cells.ng_FieldSolver, 0.0_rt);
                m_fields.alloc_init(FieldType::area_mod, Direction{2}, lev,
                                    amrex::convert(ba, Bz_nodal_flag), dm,
                                    ncomps, guard_cells.ng_FieldSolver, 0.0_rt);

                m_borrowing[lev][0] =
                    std::make_unique<amrex::LayoutData<FaceInfoBox>>(
                        amrex::convert(ba, Bx_nodal_flag), dm);
                m_borrowing[lev][1] =
                    std::make_unique<amrex::LayoutData<FaceInfoBox>>(
                        amrex::convert(ba, By_nodal_flag), dm);
                ...
                /** Venl contains the electromotive force for every mesh face,
                 * i.e. every entry is the corresponding entry in ECTRhofield
                 * multiplied by the total area (possibly with enlargement) This
                 * is only used for the ECT solver.*/
                m_fields.alloc_init(FieldType::Venl, Direction{0}, lev,
                                    amrex::convert(ba, Bx_nodal_flag), dm,
                                    ncomps, guard_cells.ng_FieldSolver, 0.0_rt);
                m_fields.alloc_init(FieldType::Venl, Direction{1}, lev,
                                    amrex::convert(ba, By_nodal_flag), dm,
                                    ncomps, guard_cells.ng_FieldSolver, 0.0_rt);
                m_fields.alloc_init(FieldType::Venl, Direction{2}, lev,
                                    amrex::convert(ba, Bz_nodal_flag), dm,
                                    ncomps, guard_cells.ng_FieldSolver, 0.0_rt);

                /** ECTRhofield is needed only by the ect
                 * solver and it contains the electromotive force density for
                 * every mesh face. The name ECTRhofield has been used to comply
                 * with the notation of the paper
                 * https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=4463918
                 * (page 9, equation 4 and below). Although it's called rho it
                 * has nothing to do with the charge density! This is only used
                 * for the ECT solver.*/
                m_fields.alloc_init(FieldType::ECTRhofield, Direction{0}, lev,
                                    amrex::convert(ba, Bx_nodal_flag), dm,
                                    ncomps, guard_cells.ng_FieldSolver, 0.0_rt);
                m_fields.alloc_init(FieldType::ECTRhofield, Direction{1}, lev,
                                    amrex::convert(ba, By_nodal_flag), dm,
                                    ncomps, guard_cells.ng_FieldSolver, 0.0_rt);
                m_fields.alloc_init(FieldType::ECTRhofield, Direction{2}, lev,
                                    amrex::convert(ba, Bz_nodal_flag), dm,
                                    ncomps, guard_cells.ng_FieldSolver, 0.0_rt);
            }
        }
    ...
    if (WarpX::electromagnetic_solver_id == ElectromagneticSolverAlgo::PSATD) {
        if (do_dive_cleaning || update_with_rho || current_correction) {
            // For the multi-J algorithm we can allocate only one rho component
            // (no distinction between old and new)
            rho_ncomps = (WarpX::do_multi_J) ? ncomps : 2 * ncomps;
        }
    }
    if (rho_ncomps > 0) {
        m_fields.alloc_init(FieldType::rho_fp, lev,
                            amrex::convert(ba, rho_nodal_flag), dm, rho_ncomps,
                            ngRho, 0.0_rt);
    }
    ...
    if (electrostatic_solver_id == ElectrostaticSolverAlgo::LabFrame ||
        ...
        m_fields.alloc_init(FieldType::phi_fp, lev,
                            amrex::convert(ba, phi_nodal_flag), dm, ncomps,
                            ngPhi, 0.0_rt);
    }

    if (m_do_subcycling && lev == 0) {
        m_fields.alloc_init(FieldType::current_store, Direction{0}, lev,
                            amrex::convert(ba, jx_nodal_flag), dm, ncomps, ngJ,
                            0.0_rt);
        m_fields.alloc_init(FieldType::current_store, Direction{1}, lev,
                            amrex::convert(ba, jy_nodal_flag), dm, ncomps, ngJ,
                            0.0_rt);
        m_fields.alloc_init(FieldType::current_store, Direction{2}, lev,
                            amrex::convert(ba, jz_nodal_flag), dm, ncomps, ngJ,
                            0.0_rt);
    }

    if (do_dive_cleaning) {
        m_fields.alloc_init(FieldType::F_fp, lev,
                            amrex::convert(ba, F_nodal_flag), dm, ncomps, ngF,
                            0.0_rt);
    }

    if (do_divb_cleaning) {
        m_fields.alloc_init(FieldType::G_fp, lev,
                            amrex::convert(ba, G_nodal_flag), dm, ncomps, ngG,
                            0.0_rt);
    }
    ...
    if (WarpX::electromagnetic_solver_id == ElectromagneticSolverAlgo::PSATD) {
        ...
        if (fft_periodic_single_box) {
            ...
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                geom[0].isPeriodic(1) // domain is periodic in z
                    && ba.size() == 1 &&
                    lev == 0, // domain is decomposed in a single box
                "The option `psatd.periodic_single_box_fft` can only be used "
                "for a periodic domain, decomposed in a single box");
#else
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
        ...
        if (field_boundary_hi[0] == FieldBoundaryType::PML &&
            !do_pml_in_domain) {
        ...
        AllocLevelSpectralSolverRZ(spectral_solver_fp, lev, realspace_ba, dm,
                                   dx);
#else
        if (!fft_periodic_single_box) {
        ...
        AllocLevelSpectralSolver(spectral_solver_fp, lev, realspace_ba, dm, dx,
                                 pml_flag_false);
#endif
#endif
    } // ElectromagneticSolverAlgo::PSATD
        ...
        m_fdtd_solver_fp[lev] = std::make_unique<FiniteDifferenceSolver>(
            electromagnetic_solver_id, dx, grid_type);
    }
    ...
    if (aux_is_nodal and grid_type != GridType::Collocated) {
        ...
        m_fields.alloc_init(FieldType::Bfield_aux, Direction{0}, lev, nba, dm,
                            ncomps, ngEB, 0.0_rt);
        m_fields.alloc_init(FieldType::Bfield_aux, Direction{1}, lev, nba, dm,
                            ncomps, ngEB, 0.0_rt);
        m_fields.alloc_init(FieldType::Bfield_aux, Direction{2}, lev, nba, dm,
                            ncomps, ngEB, 0.0_rt);

        m_fields.alloc_init(FieldType::Efield_aux, Direction{0}, lev, nba, dm,
                            ncomps, ngEB, 0.0_rt);
        m_fields.alloc_init(FieldType::Efield_aux, Direction{1}, lev, nba, dm,
                            ncomps, ngEB, 0.0_rt);
        m_fields.alloc_init(FieldType::Efield_aux, Direction{2}, lev, nba, dm,
                            ncomps, ngEB, 0.0_rt);
    } else if (lev == 0) {
        if (WarpX::fft_do_time_averaging) {
            m_fields.alias_init(FieldType::Bfield_aux, FieldType::Bfield_avg_fp,
                                Direction{0}, lev, 0.0_rt);
            m_fields.alias_init(FieldType::Bfield_aux, FieldType::Bfield_avg_fp,
                                Direction{1}, lev, 0.0_rt);
            m_fields.alias_init(FieldType::Bfield_aux, FieldType::Bfield_avg_fp,
                                Direction{2}, lev, 0.0_rt);

            m_fields.alias_init(FieldType::Efield_aux, FieldType::Efield_avg_fp,
                                Direction{0}, lev, 0.0_rt);
            m_fields.alias_init(FieldType::Efield_aux, FieldType::Efield_avg_fp,
                                Direction{1}, lev, 0.0_rt);
            m_fields.alias_init(FieldType::Efield_aux, FieldType::Efield_avg_fp,
            ...
            if (mypc->m_B_ext_particle_s == "read_from_file") {
                m_fields.alloc_init(FieldType::Bfield_aux, Direction{0}, lev,
                                    amrex::convert(ba, Bx_nodal_flag), dm,
                                    ncomps, ngEB, 0.0_rt);
                m_fields.alloc_init(FieldType::Bfield_aux, Direction{1}, lev,
                                    amrex::convert(ba, By_nodal_flag), dm,
                                    ncomps, ngEB, 0.0_rt);
                m_fields.alloc_init(FieldType::Bfield_aux, Direction{2}, lev,
                                    amrex::convert(ba, Bz_nodal_flag), dm,
                                    ncomps, ngEB, 0.0_rt);
            } else {
                // In this case, the aux grid is simply an alias of the fp grid
                // (most common case in WarpX)
            ...
            if (mypc->m_E_ext_particle_s == "read_from_file") {
                m_fields.alloc_init(FieldType::Efield_aux, Direction{0}, lev,
                                    amrex::convert(ba, Ex_nodal_flag), dm,
                                    ncomps, ngEB, 0.0_rt);
                m_fields.alloc_init(FieldType::Efield_aux, Direction{1}, lev,
                                    amrex::convert(ba, Ey_nodal_flag), dm,
                                    ncomps, ngEB, 0.0_rt);
                m_fields.alloc_init(FieldType::Efield_aux, Direction{2}, lev,
                                    amrex::convert(ba, Ez_nodal_flag), dm,
                                    ncomps, ngEB, 0.0_rt);
            } else {
                // In this case, the aux grid is simply an alias of the fp grid
    ...
    if (m_p_ext_field_params->B_ext_grid_type !=
        ...
        // These fields will be added directly to the grid, i.e. to fp, and need
        // to match the index type
        m_fields.alloc_init(
            FieldType::Bfield_fp_external, Direction{0}, lev,
            amrex::convert(
                ba, m_fields.get(FieldType::Bfield_fp, Direction{0}, lev)
                        ->ixType()),
            dm, ncomps, ngEB, 0.0_rt);
        m_fields.alloc_init(
            FieldType::Bfield_fp_external, Direction{1}, lev,
            amrex::convert(
                ba, m_fields.get(FieldType::Bfield_fp, Direction{1}, lev)
                        ->ixType()),
            dm, ncomps, ngEB, 0.0_rt);
        m_fields.alloc_init(
            FieldType::Bfield_fp_external, Direction{2}, lev,
            amrex::convert(
                ba, m_fields.get(FieldType::Bfield_fp, Direction{2}, lev)
                        ->ixType()),
            dm, ncomps, ngEB, 0.0_rt);
    }
    if (mypc->m_B_ext_particle_s == "read_from_file") {
        //  These fields will be added to the fields that the particles see, and
        //  need to match the index type
        auto* Bfield_aux_levl_0 =
            m_fields.get(FieldType::Bfield_aux, Direction{0}, lev);
        auto* Bfield_aux_levl_1 =
            m_fields.get(FieldType::Bfield_aux, Direction{1}, lev);
        auto* Bfield_aux_levl_2 =
            m_fields.get(FieldType::Bfield_aux, Direction{2}, lev);

        // Same as Bfield_fp for reading external field data
        m_fields.alloc_init(FieldType::B_external_particle_field, Direction{0},
                            lev,
                            amrex::convert(ba, Bfield_aux_levl_0->ixType()), dm,
                            ncomps, ngEB, 0.0_rt);
        m_fields.alloc_init(FieldType::B_external_particle_field, Direction{1},
                            lev,
                            amrex::convert(ba, Bfield_aux_levl_1->ixType()), dm,
                            ncomps, ngEB, 0.0_rt);
        m_fields.alloc_init(FieldType::B_external_particle_field, Direction{2},
                            lev,
                            amrex::convert(ba, Bfield_aux_levl_2->ixType()), dm,
                            ncomps, ngEB, 0.0_rt);
    }
    if (m_p_ext_field_params->E_ext_grid_type !=
        ...
        // to match the index type
        m_fields.alloc_init(
            FieldType::Efield_fp_external, Direction{0}, lev,
            amrex::convert(
                ba, m_fields.get(FieldType::Efield_fp, Direction{0}, lev)
                        ->ixType()),
            dm, ncomps, ngEB, 0.0_rt);
        m_fields.alloc_init(
            FieldType::Efield_fp_external, Direction{1}, lev,
            amrex::convert(
                ba, m_fields.get(FieldType::Efield_fp, Direction{1}, lev)
                        ->ixType()),
            dm, ncomps, ngEB, 0.0_rt);
        m_fields.alloc_init(
            FieldType::Efield_fp_external, Direction{2}, lev,
            amrex::convert(
                ba, m_fields.get(FieldType::Efield_fp, Direction{2}, lev)
                        ->ixType()),
            dm, ncomps, ngEB, 0.0_rt);
    }
    if (mypc->m_E_ext_particle_s == "read_from_file") {
        //  These fields will be added to the fields that the particles see, and
        //  need to match the index type
        auto* Efield_aux_levl_0 =
            m_fields.get(FieldType::Efield_aux, Direction{0}, lev);
        auto* Efield_aux_levl_1 =
            m_fields.get(FieldType::Efield_aux, Direction{1}, lev);
        auto* Efield_aux_levl_2 =
            m_fields.get(FieldType::Efield_aux, Direction{2}, lev);

        // Same as Efield_fp for reading external field data
        m_fields.alloc_init(FieldType::E_external_particle_field, Direction{0},
                            lev,
                            amrex::convert(ba, Efield_aux_levl_0->ixType()), dm,
                            ncomps, ngEB, 0.0_rt);
        m_fields.alloc_init(FieldType::E_external_particle_field, Direction{1},
                            lev,
                            amrex::convert(ba, Efield_aux_levl_1->ixType()), dm,
                            ncomps, ngEB, 0.0_rt);
        m_fields.alloc_init(FieldType::E_external_particle_field, Direction{2},
                            lev,
                            amrex::convert(ba, Efield_aux_levl_2->ixType()), dm,
                            ncomps, ngEB, 0.0_rt);
    }
    ...
    if (lev > 0) {
        ...
        m_fields.alloc_init(FieldType::Bfield_cp, Direction{0}, lev,
                            amrex::convert(cba, Bx_nodal_flag), dm, ncomps,
                            ngEB, 0.0_rt);
        m_fields.alloc_init(FieldType::Bfield_cp, Direction{1}, lev,
                            amrex::convert(cba, By_nodal_flag), dm, ncomps,
                            ngEB, 0.0_rt);
        m_fields.alloc_init(FieldType::Bfield_cp, Direction{2}, lev,
                            amrex::convert(cba, Bz_nodal_flag), dm, ncomps,
                            ngEB, 0.0_rt);

        // Create the MultiFabs for E
        m_fields.alloc_init(FieldType::Efield_cp, Direction{0}, lev,
                            amrex::convert(cba, Ex_nodal_flag), dm, ncomps,
                            ngEB, 0.0_rt);
        m_fields.alloc_init(FieldType::Efield_cp, Direction{1}, lev,
                            amrex::convert(cba, Ey_nodal_flag), dm, ncomps,
                            ngEB, 0.0_rt);
        m_fields.alloc_init(FieldType::Efield_cp, Direction{2}, lev,
                            amrex::convert(cba, Ez_nodal_flag), dm, ncomps,
                            ngEB, 0.0_rt);

        if (fft_do_time_averaging) {
            m_fields.alloc_init(FieldType::Bfield_avg_cp, Direction{0}, lev,
                                amrex::convert(cba, Bx_nodal_flag), dm, ncomps,
                                ngEB, 0.0_rt);
            m_fields.alloc_init(FieldType::Bfield_avg_cp, Direction{1}, lev,
                                amrex::convert(cba, By_nodal_flag), dm, ncomps,
                                ngEB, 0.0_rt);
            m_fields.alloc_init(FieldType::Bfield_avg_cp, Direction{2}, lev,
                                amrex::convert(cba, Bz_nodal_flag), dm, ncomps,
                                ngEB, 0.0_rt);

            m_fields.alloc_init(FieldType::Efield_avg_cp, Direction{0}, lev,
                                amrex::convert(cba, Ex_nodal_flag), dm, ncomps,
                                ngEB, 0.0_rt);
            m_fields.alloc_init(FieldType::Efield_avg_cp, Direction{1}, lev,
                                amrex::convert(cba, Ey_nodal_flag), dm, ncomps,
                                ngEB, 0.0_rt);
            m_fields.alloc_init(FieldType::Efield_avg_cp, Direction{2}, lev,
                                amrex::convert(cba, Ez_nodal_flag), dm, ncomps,
                                ngEB, 0.0_rt);
        }

        // Create the MultiFabs for the current
        m_fields.alloc_init(FieldType::current_cp, Direction{0}, lev,
                            amrex::convert(cba, jx_nodal_flag), dm, ncomps, ngJ,
                            0.0_rt);
        m_fields.alloc_init(FieldType::current_cp, Direction{1}, lev,
                            amrex::convert(cba, jy_nodal_flag), dm, ncomps, ngJ,
                            0.0_rt);
        m_fields.alloc_init(FieldType::current_cp, Direction{2}, lev,
                            amrex::convert(cba, jz_nodal_flag), dm, ncomps, ngJ,
                            0.0_rt);

        if (rho_ncomps > 0) {
            m_fields.alloc_init(FieldType::rho_cp, lev,
                                amrex::convert(cba, rho_nodal_flag), dm,
                                rho_ncomps, ngRho, 0.0_rt);
        }

        if (do_dive_cleaning) {
            m_fields.alloc_init(FieldType::F_cp, lev,
                                amrex::convert(cba, IntVect::TheUnitVector()),
                                dm, ncomps, ngF, 0.0_rt);
        }
        ...
        if (do_divb_cleaning) {
            if (grid_type == GridType::Collocated) {
                m_fields.alloc_init(
                    FieldType::G_cp, lev,
                    amrex::convert(cba, IntVect::TheUnitVector()), dm, ncomps,
                    ngG, 0.0_rt);
            } else // grid_type=staggered or grid_type=hybrid
            {
                m_fields.alloc_init(
                    FieldType::G_cp, lev,
                    amrex::convert(cba, IntVect::TheZeroVector()), dm, ncomps,
                    ngG, 0.0_rt);
            }
        }
        ...
        if (WarpX::electromagnetic_solver_id ==
...
#ifndef WARPX_USE_FFT
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                false, "WarpX::AllocLevelMFs: PSATD solver requires WarpX "
                       "build with spectral solver support.");
#else
            ...
            AllocLevelSpectralSolverRZ(spectral_solver_cp, lev, c_realspace_ba,
                                       dm, cdx);
#else
            c_realspace_ba.grow(ngEB);
            bool const pml_flag_false = false;
            AllocLevelSpectralSolver(spectral_solver_cp, lev, c_realspace_ba,
                                     dm, cdx, pml_flag_false);
#endif
#endif
        } // ElectromagneticSolverAlgo::PSATD
        else {
            m_fdtd_solver_cp[lev] = std::make_unique<FiniteDifferenceSolver>(
                electromagnetic_solver_id, cdx, grid_type);
        }
    }
    ...
    //
    if (lev > 0 &&
        (n_field_gather_buffer > 0 || n_current_deposition_buffer > 0 ||
        ...
        if (n_field_gather_buffer > 0 ||
            ...
            if (aux_is_nodal) {
                BoxArray const& cnba =
                    amrex::convert(cba, IntVect::TheNodeVector());
                // Create the MultiFabs for B
                m_fields.alloc_init(FieldType::Bfield_cax, Direction{0}, lev,
                                    cnba, dm, ncomps, ngEB, 0.0_rt);
                m_fields.alloc_init(FieldType::Bfield_cax, Direction{1}, lev,
                                    cnba, dm, ncomps, ngEB, 0.0_rt);
                m_fields.alloc_init(FieldType::Bfield_cax, Direction{2}, lev,
                                    cnba, dm, ncomps, ngEB, 0.0_rt);

                // Create the MultiFabs for E
                m_fields.alloc_init(FieldType::Efield_cax, Direction{0}, lev,
                                    cnba, dm, ncomps, ngEB, 0.0_rt);
                m_fields.alloc_init(FieldType::Efield_cax, Direction{1}, lev,
                                    cnba, dm, ncomps, ngEB, 0.0_rt);
                m_fields.alloc_init(FieldType::Efield_cax, Direction{2}, lev,
                                    cnba, dm, ncomps, ngEB, 0.0_rt);
            } else {
                // Create the MultiFabs for B
                m_fields.alloc_init(FieldType::Bfield_cax, Direction{0}, lev,
                                    amrex::convert(cba, Bx_nodal_flag), dm,
                                    ncomps, ngEB, 0.0_rt);
                m_fields.alloc_init(FieldType::Bfield_cax, Direction{1}, lev,
                                    amrex::convert(cba, By_nodal_flag), dm,
                                    ncomps, ngEB, 0.0_rt);
                m_fields.alloc_init(FieldType::Bfield_cax, Direction{2}, lev,
                                    amrex::convert(cba, Bz_nodal_flag), dm,
                                    ncomps, ngEB, 0.0_rt);

                // Create the MultiFabs for E
                m_fields.alloc_init(FieldType::Efield_cax, Direction{0}, lev,
                                    amrex::convert(cba, Ex_nodal_flag), dm,
                                    ncomps, ngEB, 0.0_rt);
                m_fields.alloc_init(FieldType::Efield_cax, Direction{1}, lev,
                                    amrex::convert(cba, Ey_nodal_flag), dm,
                                    ncomps, ngEB, 0.0_rt);
                m_fields.alloc_init(FieldType::Efield_cax, Direction{2}, lev,
                                    amrex::convert(cba, Ez_nodal_flag), dm,
                                    ncomps, ngEB, 0.0_rt);
            }

            AllocInitMultiFab(gather_buffer_masks[lev], ba, dm, ncomps,
                              amrex::IntVect(1), lev, "gather_buffer_masks");
            // Gather buffer masks have 1 ghost cell, because of the fact
            // that particles may move by more than one cell when using
            // subcycling.
        }

        if (n_current_deposition_buffer > 0) {
            m_fields.alloc_init(FieldType::current_buf, Direction{0}, lev,
                                amrex::convert(cba, jx_nodal_flag), dm, ncomps,
                                ngJ, 0.0_rt);
            m_fields.alloc_init(FieldType::current_buf, Direction{1}, lev,
                                amrex::convert(cba, jy_nodal_flag), dm, ncomps,
                                ngJ, 0.0_rt);
            m_fields.alloc_init(FieldType::current_buf, Direction{2}, lev,
                                amrex::convert(cba, jz_nodal_flag), dm, ncomps,
                                ngJ, 0.0_rt);
            if (m_fields.has(FieldType::rho_cp, lev)) {
                m_fields.alloc_init(FieldType::rho_buf, lev,
                                    amrex::convert(cba, rho_nodal_flag), dm,
                                    2 * ncomps, ngRho, 0.0_rt);
            }
            AllocInitMultiFab(current_buffer_masks[lev], ba, dm, ncomps,
                              amrex::IntVect(1), lev, "current_buffer_masks");
 ...
 *
 * \param[in, out] spectral_solver Vector of pointer to SpectralSolver, to point
 * to allocated spectral Maxwell solver at a given level
 * \param[in] lev                  Level at which to allocate spectral Maxwell
 * solver
 * \param[in] realspace_ba         Box array that corresponds to the
 * decomposition of the fields in real space (cell-centered; includes guard
 * cells)
 * \param[in] dm                   Indicates which MPI proc owns which box, in
 * realspace_ba
 * \param[in] dx                   Cell size along each dimension
 */
void
WarpX::AllocLevelSpectralSolverRZ (
    amrex::Vector<std::unique_ptr<SpectralSolverRZ>>& spectral_solver,
    const int lev, const amrex::BoxArray& realspace_ba,
    const amrex::DistributionMapping& dm, const std::array<Real, 3>& dx) {
    const RealVect dx_vect(dx[0], dx[2]);

    amrex::Real solver_dt = dt[lev];
    if (WarpX::do_multi_J) {
        solver_dt /= static_cast<amrex::Real>(WarpX::do_multi_J_n_depositions);
    }
    if (evolve_scheme == EvolveScheme::StrangImplicitSpectralEM) {
    ...
    auto pss = std::make_unique<SpectralSolverRZ>(
        lev, realspace_ba, dm, n_rz_azimuthal_modes, noz_fft, grid_type,
        m_v_galilean, dx_vect, solver_dt,
        ::isAnyBoundaryPML(field_boundary_lo, field_boundary_hi),
 ...
 *
 * \param[in, out] spectral_solver  Vector of pointer to SpectralSolver, to
 * point to allocated spectral Maxwell solver at a given level
 * \param[in] lev                   Level at which to allocate spectral Maxwell
 * solver
 * \param[in] realspace_ba          Box array that corresponds to the
 * decomposition of the fields in real space (cell-centered; includes guard
 ...
 * realspace_ba
 * \param[in] dx                    Cell size along each dimension
 * \param[in] pml_flag              Whether the boxes in which the solver is
 * applied are PML boxes
 */
void
WarpX::AllocLevelSpectralSolver (
    ...
    auto pss = std::make_unique<SpectralSolver>(
        lev, realspace_ba, dm, nox_fft, noy_fft, noz_fft, grid_type,
        m_v_galilean, m_v_comoving, dx_vect, solver_dt, pml_flag,
        fft_periodic_single_box, update_with_rho, fft_do_time_averaging,
        m_psatd_solution_type, J_in_time, rho_in_time, do_dive_cleaning,
        do_divb_cleaning);
    spectral_solver[lev] = std::move(pss);
}
#endif
...
amrex::XDim3
WarpX::LowerCorner (const Box& bx, const int lev,
    ...
    const amrex::Real cur_time = warpx.gett_new(lev);
    const amrex::Real time_shift =
        (cur_time + time_shift_delta - warpx.time_of_last_gal_shift);
...
#if defined(WARPX_DIM_3D)
    return {grid_min[0] + galilean_shift[0], grid_min[1] + galilean_shift[1],
            grid_min[2] + galilean_shift[2]};

#elif defined(WARPX_DIM_XZ) || defined(WARPX_DIM_RZ)
    return {grid_min[0] + galilean_shift[0],
            std::numeric_limits<Real>::lowest(),
            grid_min[1] + galilean_shift[2]};

#elif defined(WARPX_DIM_1D_Z)
    return {std::numeric_limits<Real>::lowest(),
            std::numeric_limits<Real>::lowest(),
            grid_min[0] + galilean_shift[2]};
#endif
}

amrex::XDim3
WarpX::UpperCorner (const Box& bx, const int lev,
                    const amrex::Real time_shift_delta) {
    auto& warpx = GetInstance();
    const RealBox grid_box = getRealBox(bx, lev);
...
#if defined(WARPX_DIM_3D)
    return {grid_max[0] + galilean_shift[0], grid_max[1] + galilean_shift[1],
            grid_max[2] + galilean_shift[2]};

#elif defined(WARPX_DIM_XZ) || defined(WARPX_DIM_RZ)
    return {grid_max[0] + galilean_shift[0], std::numeric_limits<Real>::max(),
            grid_max[1] + galilean_shift[1]};

#elif defined(WARPX_DIM_1D_Z)
    return {std::numeric_limits<Real>::max(), std::numeric_limits<Real>::max(),
            grid_max[0] + galilean_shift[0]};
#endif
}
...
WarpX::ComputeDivB (amrex::MultiFab& divB, int const dcomp,
    ...
    const Real dxinv = 1._rt / dx[0], dyinv = 1._rt / dx[1],
               dzinv = 1._rt / dx[2];
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(divB, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
            warpx_computedivb(i, j, k, dcomp, divBfab, Bxfab, Byfab, Bzfab,
                              dxinv, dyinv, dzinv
#ifdef WARPX_DIM_RZ
                              ,
                              rmin
...
WarpX::ComputeDivE (amrex::MultiFab& divE, const int lev) {
    if (WarpX::electromagnetic_solver_id == ElectromagneticSolverAlgo::PSATD) {
#ifdef WARPX_USE_FFT
        const ablastr::fields::VectorField Efield_aux_lev =
...
WarpX::BuildBufferMasks () {
    for (int lev = 1; lev <= maxLevel(); ++lev) {
        for (int ipass = 0; ipass < 2; ++ipass) {
            const int ngbuffer = (ipass == 0) ? n_current_deposition_buffer
                                              : n_field_gather_buffer;
            iMultiFab* bmasks = (ipass == 0) ? current_buffer_masks[lev].get()
                                             : gather_buffer_masks[lev].get();
            if (bmasks) {
                const IntVect ngtmp = ngbuffer + bmasks->nGrowVect();
                iMultiFab tmp(bmasks->boxArray(), bmasks->DistributionMap(), 1,
                              ngtmp);
                const int covered = 1;
                ...
                tmp.BuildMask(dom, period, covered, notcovered, physbnd,
                              interior);
#ifdef AMREX_USE_OMP
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
                ...
                for (MFIter mfi(*bmasks, true); mfi.isValid(); ++mfi) {
                    ...
                    BuildBufferMasksInBox(tbx, (*bmasks)[mfi], tmp[mfi],
                                          ngbuffer);
                }
            }
        }
 ...
 */
void
WarpX::BuildBufferMasksInBox (const amrex::Box tbx,
...
void
WarpX::AllocateCenteringCoefficients (
    amrex::Gpu::DeviceVector<amrex::Real>& device_centering_stencil_coeffs_x,
    amrex::Gpu::DeviceVector<amrex::Real>& device_centering_stencil_coeffs_y,
    amrex::Gpu::DeviceVector<amrex::Real>& device_centering_stencil_coeffs_z,
    const int centering_nox, const int centering_noy, const int centering_noz,
    ...
    Fornberg_stencil_coeffs_x = ablastr::math::getFornbergStencilCoefficients(
        centering_nox, a_grid_type);
    Fornberg_stencil_coeffs_y = ablastr::math::getFornbergStencilCoefficients(
        centering_noy, a_grid_type);
    Fornberg_stencil_coeffs_z = ablastr::math::getFornbergStencilCoefficients(
        centering_noz, a_grid_type);
    ...
    ablastr::math::ReorderFornbergCoefficients(host_centering_stencil_coeffs_x,
                                               Fornberg_stencil_coeffs_x,
                                               centering_nox);
    ablastr::math::ReorderFornbergCoefficients(host_centering_stencil_coeffs_y,
                                               Fornberg_stencil_coeffs_y,
                                               centering_noy);
    ablastr::math::ReorderFornbergCoefficients(host_centering_stencil_coeffs_z,
                                               Fornberg_stencil_coeffs_z,
                                               centering_noz);

    // Device vectors of stencil coefficients used for finite-order centering

    device_centering_stencil_coeffs_x.resize(
        host_centering_stencil_coeffs_x.size());
    amrex::Gpu::copyAsync(amrex::Gpu::hostToDevice,
                          host_centering_stencil_coeffs_x.begin(),
                          host_centering_stencil_coeffs_x.end(),
                          device_centering_stencil_coeffs_x.begin());

    device_centering_stencil_coeffs_y.resize(
        host_centering_stencil_coeffs_y.size());
    amrex::Gpu::copyAsync(amrex::Gpu::hostToDevice,
                          host_centering_stencil_coeffs_y.begin(),
                          host_centering_stencil_coeffs_y.end(),
                          device_centering_stencil_coeffs_y.begin());

    device_centering_stencil_coeffs_z.resize(
        host_centering_stencil_coeffs_z.size());
    amrex::Gpu::copyAsync(amrex::Gpu::hostToDevice,
    ...
    for (int idim = 0; idim < 3; ++idim) {
        if (m_fields.has(FieldType::current_store, Direction{idim}, lev)) {
            MultiFab::Copy(
                ...
                *m_fields.get(FieldType::current_fp, Direction{idim}, lev), 0,
                0, 1,
                m_fields.get(FieldType::current_store, Direction{idim}, lev)
                    ->nGrowVect());
        }
...
WarpX::isAnyParticleBoundaryThermal () {
    for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
        if (WarpX::particle_boundary_lo[idim] ==
            ParticleBoundaryType::Thermal) {
            return true;
        }
        if (WarpX::particle_boundary_hi[idim] ==
            ParticleBoundaryType::Thermal) {
            return true;
        }
    }
...
void
WarpX::AllocInitMultiFab (std::unique_ptr<amrex::iMultiFab>& mf,
                          const amrex::BoxArray& ba,
                          const amrex::DistributionMapping& dm, const int ncomp,
                          const amrex::IntVect& ngrow, const int level,
                          const std::string& name,
                          std::optional<const int> initial_value) {
    // Add the suffix "[level=level]"
    const auto name_with_suffix =
...
WarpX::getFieldDotMaskPointer (FieldType field_type, int lev,
                               ablastr::fields::Direction dir) const {
    const auto periodicity = Geom(lev).periodicity();
    switch (field_type) {
    case FieldType::Efield_fp:
        ::SetDotMask(Efield_dotMask[lev][dir],
                     m_fields.get("Efield_fp", dir, lev), periodicity);
        return Efield_dotMask[lev][dir].get();
    case FieldType::Bfield_fp:
        ::SetDotMask(Bfield_dotMask[lev][dir],
                     m_fields.get("Bfield_fp", dir, lev), periodicity);
        return Bfield_dotMask[lev][dir].get();
    case FieldType::vector_potential_fp:
        ::SetDotMask(Afield_dotMask[lev][dir],
                     m_fields.get("vector_potential_fp", dir, lev),
                     periodicity);
        return Afield_dotMask[lev][dir].get();
    case FieldType::phi_fp:
        ::SetDotMask(phi_dotMask[lev], m_fields.get("phi_fp", dir, lev),
                     periodicity);
