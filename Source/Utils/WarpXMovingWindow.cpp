...
namespace
{
    ...
    * \param[in] lev the level of the MultiFab mf
    ...
    * \param[in] track_cost_enabled flag to toggle time tracking for load balancing
    ...
    */
    void shiftMF (
        ...
        const int lev, int num_shift, int dir,
        bool safe_guard_cells, bool do_single_precision_comms,
        const bool track_cost_enabled = true,
    ...
    #pragma omp parallel if (Gpu::notInLaunchRegion())
        ...
        {
            const auto time_tracker = warpx::load_balancing::get_scoped_time_tracker(lev, mfi.index(), track_cost_enabled);
...
WarpX::MoveWindow (const int step, bool move_j)
{
    ...
    for (int lev = 0; lev <= finest_level; ++lev) {
        ...
        constexpr bool cost_flag = true;
        constexpr bool pml_cost_flag = false ; //We can't update cost for PML
        ...
        for (int dim = 0; dim < 3; ++dim) {
            ...
            ::shiftMF(*m_fields.get(FieldType::Bfield_fp, Direction{dim}, lev), geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag,
                m_p_ext_field_params->B_external_grid[dim], use_Bparser, Bfield_parser);
            ::shiftMF(*m_fields.get(FieldType::Efield_fp, Direction{dim}, lev), geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag,
            ...
            if (fft_do_time_averaging) {
                ...
                ::shiftMF(*Bfield_avg_fp[lev][dim], geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag,
                    m_p_ext_field_params->B_external_grid[dim], use_Bparser, Bfield_parser);
                ::shiftMF(*Efield_avg_fp[lev][dim], geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag,
                   m_p_ext_field_params-> E_external_grid[dim], use_Eparser, Efield_parser);
            }
            if (move_j) {
                ::shiftMF(*m_fields.get(FieldType::current_fp, Direction{dim}, lev), geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag);
            }
            if (pml[lev] && pml[lev]->ok()) {
                ...
                ::shiftMF(*pml_B, geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, pml_cost_flag);
                ::shiftMF(*pml_E, geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, pml_cost_flag);
            }
#if (defined WARPX_DIM_RZ) && (defined WARPX_USE_FFT)
            ...
            if (pml_rz[lev] && dim < 2) {
                ...
                ::shiftMF(*pml_rz_B, geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, pml_cost_flag, 0.0_rt, false, amrex::ParserExecutor<3>{}, PMLRZ_flag);
                ::shiftMF(*pml_rz_E, geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, pml_cost_flag, 0.0_rt, false, amrex::ParserExecutor<3>{}, PMLRZ_flag);
            }
#endif
            if (lev > 0) {
                // coarse grid
                ::shiftMF(*m_fields.get(FieldType::Bfield_cp, Direction{dim}, lev), geom[lev-1], lev, num_shift_crse, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag,
                    m_p_ext_field_params->B_external_grid[dim], use_Bparser, Bfield_parser);
                ::shiftMF(*m_fields.get(FieldType::Efield_cp, Direction{dim}, lev), geom[lev-1], lev, num_shift_crse, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag,
                    m_p_ext_field_params->E_external_grid[dim], use_Eparser, Efield_parser);
                ::shiftMF(*m_fields.get(FieldType::Bfield_aux, Direction{dim}, lev), geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag);
                ::shiftMF(*m_fields.get(FieldType::Efield_aux, Direction{dim}, lev), geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag);
                if (fft_do_time_averaging) {
                    ...
                    ::shiftMF(*Bfield_avg_cp[lev][dim], geom[lev-1], lev, num_shift_crse, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag,
                        m_p_ext_field_params->B_external_grid[dim], use_Bparser, Bfield_parser);
                    ::shiftMF(*Efield_avg_cp[lev][dim], geom[lev-1], lev, num_shift_crse, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag,
                        m_p_ext_field_params->E_external_grid[dim], use_Eparser, Efield_parser);
                }
                if (move_j) {
                    ::shiftMF(*m_fields.get(FieldType::current_cp, Direction{dim}, lev), geom[lev-1], lev, num_shift_crse, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag);
                }
                if (do_pml && pml[lev]->ok()) {
                    ...
                    ::shiftMF(*pml_B_cp, geom[lev-1], lev, num_shift_crse, dir, m_safe_guard_cells, do_single_precision_comms, pml_cost_flag);
                    ::shiftMF(*pml_E_cp, geom[lev-1], lev, num_shift_crse, dir, m_safe_guard_cells, do_single_precision_comms, pml_cost_flag);
                }
            }
        }
        ...
        {
            // Fine grid
            ::shiftMF(*m_fields.get(FieldType::F_fp, lev), geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag);
            if (lev > 0)
            {
                // Coarse grid
                ::shiftMF(*m_fields.get(FieldType::F_cp, lev), geom[lev-1], lev, num_shift_crse, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag);
            }
        }
        ...
        if (do_pml_dive_cleaning)
        {
            ...
            if (do_pml && pml[lev]->ok())
            {
                ...
                ::shiftMF(*pml_F, geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, pml_cost_flag);
            }
            if (lev > 0)
            {
                // Coarse grid
                if (do_pml && pml[lev]->ok())
                {
                    ...
                    ::shiftMF(*pml_F, geom[lev-1], lev, num_shift_crse, dir, m_safe_guard_cells, do_single_precision_comms, pml_cost_flag);
                }
            }
        }
        ...
        {
            // Fine grid
            ::shiftMF(*m_fields.get(FieldType::G_fp, lev), geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag);
            if (lev > 0)
            {
                // Coarse grid
                ::shiftMF(*m_fields.get(FieldType::G_cp, lev), geom[lev-1], lev, num_shift_crse, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag);
            }
        }
        ...
        if (do_pml_divb_cleaning)
        {
            ...
            if (do_pml && pml[lev]->ok())
            {
                ...
                ::shiftMF(*pml_G, geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, pml_cost_flag);
            }
            if (lev > 0)
            {
                // Coarse grid
                if (do_pml && pml[lev]->ok())
                {
                    ...
                    ::shiftMF(*pml_G, geom[lev-1], lev, num_shift_crse, dir, m_safe_guard_cells, do_single_precision_comms, pml_cost_flag);
                }
            }
        }
        ...
        if (move_j) {
            if (m_fields.has(FieldType::rho_fp, lev)) {
                // Fine grid
                ::shiftMF(*m_fields.get(FieldType::rho_fp,lev), geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag);
                if (lev > 0){
                    // Coarse grid
                    ::shiftMF(*m_fields.get(FieldType::rho_cp,lev), geom[lev-1], lev, num_shift_crse, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag);
                }
            }
        }
        ...
        if (do_fluid_species) {
            ...
            for (int i=0; i<n_fluid_species; i++) {
                ...
                ::shiftMF( *m_fields.get(fl.name_mf_N, lev), geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag );
                ::shiftMF( *m_fields.get(fl.name_mf_NU, Direction{0}, lev), geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag );
                ::shiftMF( *m_fields.get(fl.name_mf_NU, Direction{1}, lev), geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag );
                ::shiftMF( *m_fields.get(fl.name_mf_NU, Direction{2}, lev), geom[lev], lev, num_shift, dir, m_safe_guard_cells, do_single_precision_comms, cost_flag );
            }
        }
    }
