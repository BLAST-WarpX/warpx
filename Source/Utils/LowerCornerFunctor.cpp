...
LowerCornerFunctor::LowerCornerFunctor(
    const amrex::Vector<amrex::Real> cur_time,
    const amrex::Real time_of_last_gal_shift,
...
LowerCornerFunctor::operator()(
    const amrex::Box& bx, const int lev, const amrex::Real time_shift_delta) const
{

    const auto gm_lev = m_geom[lev];
    const auto grid_box = amrex::RealBox{bx, gm_lev.CellSize(), gm_lev.ProbLo()};
    const amrex::Real* grid_min = grid_box.lo();

    const amrex::Real cur_time = m_cur_time[lev];
    const amrex::Real time_shift =
        (cur_time + time_shift_delta - m_time_of_last_gal_shift);
    amrex::Array<amrex::Real,3> galilean_shift = { m_v_galilean[0]*time_shift,
